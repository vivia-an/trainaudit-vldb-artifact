...
def check(conn) -> RuleResult:
    ...
    for event_id, payload_str in rows:
        ...
        if not cls:  # ABLATED[no_precond:T1-grad-flow-block-uniformity]: dropped _SKIP_CLASSES per RECON_S2 §A
            continue
        name = p.get("module_name", "")
        bidx = _block_index(name)
        # ABLATED[no_precond:T1-grad-flow-block-uniformity]: dropped `bidx is None` skip
        if bidx is None: bidx = -1  # ABLATED: stub block-idx (RECON_S2 §A)
