...
def check(conn) -> RuleResult:
    rows = conn.execute("""
        SELECT event_id, payload FROM events
         ...
         ORDER BY event_id
    """).fetchall()
    # ABLATED[no_precond:T0-optim-step-counter-monotonic]: dropped `<2` threshold
    # ABLATED[no_precond:T0-optim-step-counter-monotonic]: RECON_S2 §A row "T0-optim-step-counter-monotonic"
