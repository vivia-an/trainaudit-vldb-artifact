...
def check(conn) -> RuleResult:
    ...
    for (p,) in bld:
        ...
        if declared is not None:
            break
    if declared is None:  # ABLATED[no_precond:T1-comm-dtype-matches-training]: treat absent declared as bf16
        declared = "bfloat16"  # ABLATED: was passive return; RECON_S2 §A row "T1-comm-dtype-matches-training"
    ...
    for event_id, payload_str in rows:
        try:
            ...
            continue
        op = p.get("op") or ""
        # ABLATED[no_precond:T1-comm-dtype-matches-training]: dropped `_GRADIENT_COMM_OPS` skip
        # ABLATED[no_precond:T1-comm-dtype-matches-training]: RECON_S2 §A row "T1-comm-dtype-matches-training"
