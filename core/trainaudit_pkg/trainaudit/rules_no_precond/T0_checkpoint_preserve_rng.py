        ...
        if isinstance(m, dict):
            ...
            for prm in m.get("parameters", []):
                if "dropout" in (prm.get("name") or "").lower():
                    has_dropout = True
                    break
    # ABLATED[no_precond:T0-checkpoint-preserve-rng]: dropped `if not has_dropout: return passive`
    # ABLATED[no_precond:T0-checkpoint-preserve-rng]: RECON_S2 §A row "T0-checkpoint-preserve-rng"
    # ABLATED[no_precond]: 3-line padding to preserve line numbers
