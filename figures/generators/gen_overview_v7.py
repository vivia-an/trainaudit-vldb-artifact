...
# ================= swim lanes =================
box(1, 62, 106.5, 39, fc=BLUE_BG, ec=BLUE, lw=1.3, r=1.8, z=1)
txt(3, 98.6, 'OFFLINE  —  mine once per framework', fs=6.4, w='bold', c=BLUE, ha='left')

box(1, 26, 106.5, 33.5, fc=ORANGE_BG, ec=ORANGE, lw=1.3, r=1.8, z=1)
txt(3, 57.4, 'ONLINE  —  every training job · fully deterministic · no LLM',
    fs=6.4, w='bold', c=ORANGE, ha='left')

box(110.5, 26, 32.5, 75, fc=HUB_BG, ec=DARK, lw=1.3, r=1.8, z=1)
...
box(30.5, 63.5, 76, 33, fc='#f9fbfe', ec=DARK, lw=1.0)
...
# FSM stage-node chain
txt(32.5, 70.9, 'five-stage FSM', fs=3.9, c=GRAY, ha='left')
fsm = [('$S_1$', 'gap\nanalysis', '#6f8fc4'), ('$S_2$', 'evidence\ngathering', '#7fae9e'),
       ('$S_3$', 'synthesis\n+attack', '#c48a8a'), ('$S_4$', 'persist', '#a795c2'),
       ('$S_5$', 'report', '#c2a370')]
for k, (sn, lb, cc) in enumerate(fsm):
    cx = 34.7 + k * 6.4
    ax.add_patch(Circle((cx, 68.3), 1.55, fc=cc, ec='none', zorder=5))
    txt(cx, 68.3, sn, fs=3.6, c='white', w='bold', z=6)
    txt(cx, 65.2, lb, fs=3.0, c=GRAY, ls=1.1)
    if k < 4:
        arrow((cx + 1.8, 68.3), (cx + 4.4, 68.3), c='#9aa5b5', lw=0.6, ms=4)
...
pill(113.3, 29.8, 27, 3.2, '', '#eef6ee', GREEN)
txt(126.8, 31.4, '$\\checkmark$  0 false positives over 764 clean evals', fs=3.8, c=GREEN, z=6)
...
box(3, 27.5, 42.5, 29, fc='#fffdf9', ec=DARK, lw=1.0)
...
box(63.5, 27.5, 43, 29, fc='#fffdf9', ec=DARK, lw=1.0)
...
txt(65.5, 33.9, 'empty result = holds\nnon-empty result IS the violation set:',
    fs=4.0, style='italic', ha='left', ls=1.35)
txt(65.5, 29.6, 'coarse$\\rightarrow$fine: checksum $\\rightarrow$ module\n$\\rightarrow$ parameter $\\rightarrow$ bitwise',
    fs=3.6, c=GRAY, ha='left', ls=1.3)
# violation set mini-table (P3 buggy run)
_tx, _cols = 85.5, [('param', 9.0), ('rank', 3.4), ('cksum', 6.6)]
...
for i, (t, s, c) in enumerate(ex):
    ...
    if i == len(ex) - 1:
        txt(x + cw / 2, 18.0, 'same rule intercepted a 256$\\times$H200 fault', fs=4.0)
        txt(x + cw / 2, 15.9, '$\\approx$43,000 GPU-hours saved', fs=4.4, w='bold', c=RED)
    else:
...
fig.savefig('fig_overview_v7.pdf')
fig.savefig('/tmp/fig_overview_v7.png', dpi=220)
print('done')
