#!/usr/bin/env python3
"""v21 = v20 + de-overlap text/arrows/icons (no label change).
Labels identical to v20. No Agents / 0-4 / 43k / fake hex.
"""
import matplotlib
matplotlib.use('Agg')
...
# isolate compose row (inputs to miner) as its own nested block
...
for px, ic, lb in plus_cards:
    ...
    ic(px + 2.35, 82.9, 0.72, BLUE)          # icon clear of text
    txt(px + 5.0, 82.9, lb, fs=3.45, ls=1.18, ha='left')  # left-aligned, no icon collide
for gx in (46.2, 65.2, 84.2):
    txt(gx, 82.9, '+', fs=6.2, w='bold', c=GRAY_LINE)
# compose row → pipeline (continuous spine, not a stub)
arrow((65.5, 80.4), (65.5, 79.2), c=BLUE_SOFT, lw=LW_THICK, ms=9, halo=True)
...
for x, name, sub, ec, lw, fc in steps:
    ...
    if name == 'Scope':
        icon_target(x + 3.0, 74.35, 0.8, DARK)
    elif name == 'Ground':
        icon_layers(x + 3.0, 74.35, 0.75, DARK)
    elif name == 'Accept':
        icon_checkc(x + 3.15, 74.35, 0.8, DARK)
    elif name == 'Construct':
        chip(x + 2.7, 74.35, 1.15, RED)
        icon_shield(x + 2.7, 74.4, 0.65, 'white')
...
fig.savefig('fig_overview_v21.pdf')
fig.savefig('/tmp/fig_overview_v21.png', dpi=240)
print('done v21')
