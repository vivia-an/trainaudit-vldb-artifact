#!/usr/bin/env python3
"""v14 = v13 bloodline-safe + QUITE stroke ladder / pixel line DNA.
Labels identical to v13; only widths, colors, joins, arrowheads, corner radii.
"""
import matplotlib
matplotlib.use('Agg')
...
# QUITE pixel DNA (fills/strokes from Fig.3 path extract)
BLUE = '#306092'          # 0.188,0.377,0.572
BLUE_SOFT = '#6C8EBF'     # 0.424,0.557,0.749
BLUE_DEEP = '#23445D'     # 0.137,0.267,0.365
BLUE_BG = '#F5F5F5'       # QUITE near-white panel
ORANGE = '#C05621'        # online lane (domain accent; keep split)
ORANGE_BG = '#FBF3EA'
ORANGE_SOFT = '#E8A87C'
RED = '#B85450'           # 0.722,0.329,0.314
RED_DEEP = '#AE4132'      # 0.682,0.255,0.196
GREEN = '#1AABA8'         # 0.102,0.671,0.659 teal checks
GREEN_BG = '#E8F7F6'
DARK = '#2C2C2C'          # 0.172
GRAY = '#666666'          # 0.4
GRAY_LINE = '#9A9A9A'
CARD = '#FFFFFF'
HUB_BG = '#F5F5F5'
LAVENDER = '#E1D5E7'      # 0.882,0.835,0.906
PURPLE = '#9673A6'        # 0.588,0.451,0.651
MINT = '#D5E8D4'
PEACH = '#FAD9D5'         # 0.98,0.851,0.835
...
def box(x, y, w, h, fc=CARD, ec=DARK, lw=None, r=0.9, ls='-', z=2):
    if lw is None:
        lw = LW_THIN
...
def cyl(x, y, w, h, fc='#dbe7f3', ec=BLUE, z=3):
    ...
    ax.add_patch(Ellipse((x + w / 2, y + h * 0.88), w, h * 0.24, fc=fc, ec=ec, lw=LW_THIN, zorder=z + 2))
...
def shadow(x, y, w, h, r=0.9):
    # dual-layer soft lift (print-safe, not muddy)
    ax.add_patch(FancyBboxPatch((x + 0.45, y - 0.55), w, h,
                 boxstyle=f'round,pad=0,rounding_size={r}',
                 fc='#000000', ec='none', alpha=0.06, zorder=1.55))
    ax.add_patch(FancyBboxPatch((x + 0.2, y - 0.25), w, h,
                 boxstyle=f'round,pad=0,rounding_size={r}',
                 fc='#000000', ec='none', alpha=0.04, zorder=1.6))
...
def chip(cx, cy, s, c):
    ...
    ax.add_patch(FancyBboxPatch((cx - s + 0.15, cy - s + 0.15), 2 * s - 0.3, 0.5,
                 boxstyle='round,pad=0,rounding_size=0.18',
                 fc='white', ec='none', alpha=0.28, zorder=5.6))


def pill(x, y, w, h, t, fc, ec, tc=DARK, fs=3.8):
    box(x, y, w, h, fc=fc, ec=ec, lw=LW_HAIR, r=min(h / 2, 1.0), z=4)
    txt(x + w / 2, y + h / 2, t, fs=fs, family='monospace', c=tc, z=5)


def checkbox(x, y, sz, c):
    ax.add_patch(FancyBboxPatch((x, y), sz, sz, boxstyle='round,pad=0,rounding_size=0.12',
                                fc='white', ec=c, lw=LW_THIN, zorder=5))
    ax.plot([x + 0.18 * sz, x + 0.42 * sz, x + 0.88 * sz],
            [y + 0.48 * sz, y + 0.12 * sz, y + 0.82 * sz], color=c, lw=LW_MID, zorder=6,
...
box(25, 8.0, 118, 89, fc='#FFFFFF', ec='#000000', lw=LW_THICK, r=1.2, z=1.8)
...
ax.add_patch(Rectangle((27, 90.6), 79, 2.1, fc='#E8EEF6', ec='none', zorder=2))
ax.plot([27, 106], [90.6, 90.6], color=BLUE_SOFT, lw=LW_HAIR, alpha=0.55, zorder=2.1)
txt(27.5, 91.65, 'OFFLINE — mine once per framework', fs=5.7, w='bold', c=BLUE, ha='left')
ax.add_patch(Rectangle((27, 49.6), 79, 2.1, fc='#F8EDE0', ec='none', zorder=2))
ax.plot([27, 106], [49.6, 49.6], color=ORANGE, lw=LW_HAIR, alpha=0.55, zorder=2.1)
...
box(27, 54.5, 79, 35.5, fc='#FAFAFA', ec=DARK, lw=LW_MID, r=1.0)
badge(30.6, 87.2, '1')
...
for px, ic, lb in plus_cards:
    box(px, 80.6, 15.5, 4.6, fc='white', ec=BLUE_SOFT, lw=LW_THIN, r=0.55)
...
for x, name, sub, ec, lw, fc in steps:
    box(x, 66.6, 16.5, 9.5, fc=fc, ec=ec, lw=(LW_MID if ec is RED else LW_THIN), r=0.7)
...
box(108.5, 46.5, 32.5, 43.5, fc=HUB_BG, ec=DARK, lw=LW_MID, r=1.0)
...
box(110.5, 55.5, 28.5, 21.0, fc='white', ec=DARK, lw=LW_THIN, r=0.85)
...
box(27, 16.0, 36.5, 33.5, fc='#FFFAF5', ec=DARK, lw=LW_MID, r=1.0)
badge(30.6, 46.8, '2')
...
box(75.5, 16.0, 31.0, 33.5, fc='#FFFAF5', ec=DARK, lw=LW_MID, r=1.0)
badge(79.1, 46.8, '3')
...
box(76.8, 27.2, 13.6, 14.6, fc=MINT, ec='#82B366', lw=LW_THIN, r=0.7, ls=(0, (2.2, 1.6)))
...
box(91.4, 27.2, 14.0, 14.6, fc='#36393D', ec='#2C2C2C', lw=LW_THIN, r=0.7)
...
box(108.5, 11.0, 32.5, 33.0, fc=PEACH, ec=RED, lw=LW_MID, r=0.9, ls=(0, (2.8, 1.8)))
...
# ================= legend =================
ax.add_patch(Rectangle((25, 2.4), 117.5, 3.8, fc='#F5F5F5', ec='#B3B3B3', lw=LW_HAIR, zorder=2))
...
fig.savefig('fig_overview_v14.pdf')
fig.savefig('/tmp/fig_overview_v14.png', dpi=220)
print('done')
