#!/usr/bin/env python3
"""v22 = v21 + natural layout + semantic icons (no label change).
Labels identical to v21. No Agents / 0-4 / 43k / fake hex.
"""
import matplotlib
matplotlib.use('Agg')
...
# ================= left bay: catalog + source (isolated offline-inputs block) =================
dashed_group(0.5, 49.8, 23.6, 44.5, '#E8F0F8', label=None, lc=BLUE_SOFT)
...
# dual offline inputs → Miner (no orphan Framework source)
arrow((23.0, 82.5), (28.5, 83.0), c=BLUE_SOFT, lw=LW_THICK, ms=10, halo=True)
arrow((23.0, 61.5), (28.5, 72.0), c=BLUE_SOFT, lw=LW_MID, ms=9, cs='arc3,rad=0.12', halo=True)

# ================= Invariant Miner (offline row) — compressed FSM band =================
shadow(27, 53.2, 79, 36.5)
box(27, 53.2, 79, 36.5, fc=CARD, ec='#111111', lw=LW_MID + 0.15, r=1.2)
badge(30.6, 86.9, '1')
txt(33.6, 86.9, 'Invariant Miner', fs=6.6, w='bold', c=BLUE_DEEP, ha='left')
chip(53.4, 86.9, 1.55, BLUE)
icon_gear(53.4, 86.9, 0.9, 'white')
txt(104.8, 86.9, 'LLM-instantiated candidates, untrusted until verified',
    fs=3.7, style='italic', c=GRAY, ha='right')  # clear of gear chip

# compose row — even pitch (natural grid); semantic icons
dashed_group(28.3, 80.15, 76.4, 5.4, '#F2F6FC', label=None, lc=BLUE_SOFT)
# catalog=book, source=code, evidence=layers, gap=missing-ring
...
for i, (ic, lb) in enumerate(plus_cards):
    ...
    box(px, 80.55, _plus_w, 4.7, fc='#FFFFFF', ec=BLUE_SOFT, lw=LW_THIN, r=0.75)
    ic(px + 2.4, 82.9, 0.72, BLUE)
    txt(px + 5.05, 82.9, lb, fs=3.4, ls=1.32, ha='left')  # air between two lines
for i in range(3):
    gx = _plus_x0 + (i + 1) * _plus_w + i * _plus_gap + _plus_gap / 2
    txt(gx, 82.9, '+', fs=6.2, w='bold', c=GRAY_LINE)
# compose → pipeline centered under row
arrow((66.2, 80.4), (66.2, 79.2), c=BLUE_SOFT, lw=LW_THICK, ms=9, halo=True)
...
for x, name, sub, ec, lw, fc in steps:
    ...
    # title right of icon — avoid icon/title collide (esp. Construct)
    txt(x + 9.5, 74.35, name, fs=5.55, w='bold', c=ec)
    ax.plot([x + 2.4, x + 14.0], [73.15, 73.15], color=ec, lw=LW_HAIR, alpha=0.55, zorder=5)
    txt(x + 8.15, 69.45, sub, fs=3.65, ls=1.28)
    if name == 'Scope':
        icon_target(x + 3.0, 74.35, 0.8, DARK)       # scope = aim
    elif name == 'Ground':
        icon_anchor(x + 3.0, 74.35, 0.78, DARK)      # ground = anchor evidence
    elif name == 'Accept':
        icon_checkc(x + 3.15, 74.35, 0.8, DARK)      # accept = pass
    elif name == 'Construct':
        chip(x + 2.7, 74.35, 1.15, RED)
        icon_bolt(x + 2.7, 74.4, 0.72, 'white')      # attack = bolt
for x0 in (45.7, 64.95, 84.25):
    arrow((x0, 71.55), (x0 + 2.35, 71.55), c=BLUE_SOFT, lw=LW_MID, ms=7)

# reject loop — dedicated band BETWEEN step cards and FSM (no cross-text)
arrow((95.0, 66.75), (95.0, 65.55), c=RED_DEEP, lw=LW_THIN, ls=(0, (2.4, 1.5)), style='-')
arrow((95.0, 65.55), (37.3, 65.55), c=RED_DEEP, lw=LW_THIN, ls=(0, (2.4, 1.5)), style='-')
arrow((37.3, 65.55), (37.3, 66.7), c=RED_DEEP, lw=LW_THIN, ls=(0, (2.4, 1.5)))
red_x(99.3, 65.55, 0.5)
txt(66.0, 64.65, 'reject $\\rightarrow$ next iteration', fs=3.65, c=RED, style='italic')

# FSM block — lower top so reject label has clear air; wider stage pitch
dashed_group(28.35, 53.45, 76.3, 10.0, '#F0F4FA', label=None, lc=BLUE_SOFT)
txt(28.9, 61.75, 'five-stage FSM', fs=3.8, c=GRAY, ha='left')
...
_fsm_r, _fsm_y, _fsm_dx = 1.8, 59.55, 9.05   # wider pitch → labels don't collide
for k, (sn, lb, cc) in enumerate(fsm):
    cx = 40.6 + k * _fsm_dx
    ax.add_patch(Circle((cx, _fsm_y), _fsm_r, fc=cc, ec='#222222', lw=LW_HAIR, zorder=5))
    txt(cx, _fsm_y - 0.05, sn, fs=5.2, c='white', w='bold', z=6)
    txt(cx, _fsm_y - 2.75, lb, fs=3.1, c=DARK)
    if k < 4:
...
cred = [(29.5, 'decisive counterexample gate'),
        ...
        (83.5, 'Conf$(c) \\geq 0.8$')]
for cx0, tt in cred:
    checkbox(cx0, 54.25, 1.2, GREEN)
    txt(cx0 + 1.9, 54.85, tt, fs=3.55, c='#0A6A68', ha='left')

# Accept → Library; "accepted" above arrow (no sit-on-curve)
arrow((103.2, 71.7), (110.2, 76.8), c=BLUE_SOFT, lw=LW_THICK, ms=11, cs='arc3,rad=-0.16', halo=True)
txt(106.2, 80.55, 'accepted', fs=4.0, c=BLUE, style='italic', ha='right')

# ================= Verified Constraint Library (right column) =================
shadow(108.5, 46.5, 32.5, 43.5)
box(108.5, 46.5, 32.5, 43.5, fc=CARD, ec='#111111', lw=LW_MID + 0.15, r=1.2)
...
# schema cue INSIDE library (clear of ONLINE banner / FP pill)
txt(124.7, 54.0, 'compiled to parameterized SQL at job start', fs=3.4, c=ORANGE, w='bold')
txt(124.7, 52.35, 'accepted rules determine trace schema (S0–S6)',
    fs=3.3, c=ORANGE, style='italic')
pill(110.5, 48.2, 28.5, 3.0, '', GREEN_BG, GREEN)
txt(124.7, 49.7, '$\\checkmark$  0 false positives over 764 clean evals', fs=3.6, c='#0A6A68', z=6)
# Library → Online arcs: clear of banner text / no text-under-arrow
arrow((108.5, 50.6), (55.0, 47.6), c=ORANGE, lw=LW_MID, ms=10, cs='arc3,rad=0.10', halo=True)
arrow((118.5, 46.3), (106.6, 33.5), c=ORANGE, lw=LW_THICK, ms=11, cs='arc3,rad=0.20', halo=True)

# ================= Data Collector (online row) =================
shadow(27, 14.8, 36.5, 33.2)
box(27, 14.8, 36.5, 33.2, fc=CARD, ec='#111111', lw=LW_MID + 0.15, r=1.2)
badge(30.6, 45.5, '2')
txt(33.6, 45.5, 'Data Collector', fs=6.45, w='bold', c=ORANGE, ha='left')
chip(55.2, 45.5, 1.45, ORANGE)
icon_funnel(55.2, 45.5, 0.82, 'white')   # collector = funnel
...
# DuckDB bridge — inset so dashed frame doesn't kiss ❷/❸ borders
dashed_group(64.0, 18.8, 10.8, 19.6, '#FFF4E8', label=None, lc=ORANGE)
cyl(65.2, 26.4, 8.4, 11.0, fc='#F5E6D0', ec=ORANGE)
for _yy in (30.0, 32.8):
    ax.plot([66.0, 72.8], [_yy, _yy], color=ORANGE, lw=LW_HAIR, alpha=0.45, zorder=4)
txt(69.4, 23.8, 'DuckDB\ntrace DB', fs=4.2, w='bold', c=ORANGE, ls=1.2)
txt(69.4, 19.7, 'two-step\nsliding window', fs=3.45, c=GRAY, ls=1.2)
arrow((63.5, 31.8), (63.95, 31.8), c=ORANGE, lw=LW_THICK, ms=8, halo=True)
arrow((74.85, 31.8), (75.45, 31.8), c=ORANGE, lw=LW_THICK, ms=8, halo=True)

# ================= Verifier =================
shadow(75.5, 14.8, 31.0, 33.2)
box(75.5, 14.8, 31.0, 33.2, fc=CARD, ec='#111111', lw=LW_MID + 0.15, r=1.2)
badge(79.1, 45.5, '3')
txt(82.1, 45.5, 'Verifier', fs=6.45, w='bold', c=ORANGE, ha='left')
chip(92.4, 45.5, 1.45, ORANGE)
icon_verify(92.4, 45.5, 0.82, 'white')   # verifier = magnify+check
...
txt(92.7, 36.0, 'WHERE', fs=3.65, ha='left', family='monospace', c=_KW, w='bold')
txt(96.55, 36.0, '$\\pi_{topo}\\wedge\\pi_{precond}$', fs=3.5, ha='left', c=_PI, w='bold')
txt(92.7, 33.5, 'GROUP BY', fs=3.65, ha='left', family='monospace', c=_KW, w='bold')
txt(93.3, 31.35, 'param, step', fs=3.65, ha='left', family='monospace', c=_TX, w='bold')
txt(92.7, 29.0, 'HAVING', fs=3.65, ha='left', family='monospace', c=_KW, w='bold')
txt(96.7, 29.0, '$\\pi_{schema}$', fs=3.5, ha='left', c=_PI, w='bold')
txt(93.3, 26.95, 'violated', fs=3.6, ha='left', family='monospace', c=_TX, w='bold')
txt(91.0, 23.2, 'empty result = holds · non-empty IS the violation set',
    fs=3.6, style='italic')
txt(91.0, 19.7, 'coarse$\\rightarrow$fine: checksum$\\rightarrow$module$\\rightarrow$param$\\rightarrow$bitwise',
    fs=3.4, c=GRAY)
# Verifier → violation panel (continuous online→output)
arrow((106.5, 33.3), (108.4, 33.3), c=RED_DEEP, lw=LW_THICK, ms=11, halo=True)
...
# panel → left-bay artifact (same violation story, no dangling output)
arrow((108.5, 11.8), (23.5, 17.5), c=DARK, lw=LW_THICK, ms=11, cs='arc3,rad=-0.04', halo=True)

# ================= left bay: P3 violation report (isolated online-output block) =================
dashed_group(0.5, 11.2, 23.6, 32.0, '#FBF0EC', label=None, lc=ORANGE_SOFT)
...
fig.savefig('fig_overview_v22.pdf')
fig.savefig('/tmp/fig_overview_v22.png', dpi=240)
print('done v22')
