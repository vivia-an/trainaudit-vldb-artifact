#!/usr/bin/env python3
"""v32 = v31 + P0 visual polish (no paper-fact change).
Lib dual-out edge tags schema/SQL; Lib→Collector below ONLINE band;
under-lane arrow lightened. No Agents / 0-4 / 43k / fake hex.
"""
import matplotlib
matplotlib.use('Agg')
...
txt(CX, OFF_Y + 15.2, '35 templates · 13 fault classes', fs=3.25, c=GRAY)
...
_lt = OFF_Y + OFF_H
cyl(_rc - 4.1, _lt - 5.5, 8.2, 4.0, fc='#EDE6D8', ec=DARK)
txt(_rc, _lt - 7.6, 'Verified Constraint Library $\\mathcal{P}$', fs=4.7, w='bold', c=BLUE_DEEP)
# subtitle on white card only — keep ≥1.4 clear above P3 charcoal header
txt(_rc, _lt - 9.25, 'guarded relational constraints', fs=3.55, style='italic', c=GRAY)
_p3h = 14.8
_p3y = _lt - 11.0 - _p3h          # top = _lt - 11.0 → air under subtitle
shadow(RX + 2.0, _p3y, RW - 4.0, _p3h, r=0.95)
box(RX + 2.0, _p3y, RW - 4.0, _p3h, fc='#FFFFFF', ec='#222222', lw=LW_THIN, r=0.95)
box(RX + 2.0, _p3y + _p3h - 4.0, RW - 4.0, 4.0, fc='#1E242C', ec='none', r=0.95, z=3)
ax.add_patch(Rectangle((RX + 2.0, _p3y + _p3h - 4.0), RW - 4.0, 1.8, fc='#1E242C', ec='none', zorder=3))
txt(_rc, _p3y + _p3h - 1.3, 'P3 · cross-rank replication', fs=4.35, w='bold', c='white', z=5)
txt(_rc, _p3y + _p3h - 3.0, '(SwitchMLP router sync)', fs=3.35, w='bold', c='#B8C4D8', z=5)
txt(RX + 3.5, _p3y + 4.7, '$\\pi_{topo}$:  TP > 1\n$\\pi_{precond}$:  tpl = False\n   (not tensor-parallel sharded)\n$\\pi_{schema}$:  replicas share\n   checksum',
    fs=3.6, ha='left', ls=1.26)
txt(_rc, OFF_Y + 5.5, 'compiled to parameterized SQL at job start', fs=3.2, c=ORANGE, w='bold')
txt(_rc, OFF_Y + 4.05, 'accepted rules determine trace schema (S0–S6)',
    fs=3.1, c=ORANGE, style='italic')
pill(RX + 2.0, OFF_Y + 0.95, RW - 4.0, 2.5, '', GREEN_BG, GREEN)
txt(_rc, OFF_Y + 2.2, '$\\checkmark$  0 false positives over 764 clean evals', fs=3.35, c='#0A6A68', z=6)
# Lib → Collector / Lib → Verifier drawn after ❷❸ boxes exist (P0 paths + tags)

# ================= Data Collector / DuckDB / Verifier — gutters = offline arrow span =====
_oy, _oh = ON_Y, ON_H
_ot = _oy + _oh
# short DuckDB gutters (visible shaft, no layout push / left overflow)
_c0, _cw = 27.0, 35.8          # Collector → 62.8
_db0, _dbw = 64.6, 10.2        # DuckDB  → 74.8  (gap 1.8)
_v0, _vw = 76.6, 29.8          # Verifier → 106.4 (gap 1.8; →RX gap 2.1)
# Lib→Collector: restore mild arc (v31); keep edge tag only
arrow((RX, OFF_Y + 1.5), (55.0, ON_Y + ON_H - 0.5), c=ORANGE, lw=LW_MID, ms=10,
      cs='arc3,rad=0.10', halo=True)
txt(72.0, (OFF_Y + 1.5 + ON_Y + ON_H - 0.5) / 2 + 0.6, 'schema',
    fs=3.35, c=ORANGE, style='italic', z=6)
...
chip(_c0 + 30.2, _ot - 2.2, 1.35, ORANGE)
icon_funnel(_c0 + 30.2, _ot - 2.2, 0.76, 'white')
...
for i, a in enumerate(anchors):
    ...
    txt(x + w / 2 + 0.15, _chev_mid, a, fs=3.3, family='monospace', w='bold', z=5)
...
# mid-band (scheme C): paper L439 — coords unrolled; fills air above S0
_mid_y = _chev_mid - 9.35
dashed_group(_c0 + 2.0, _mid_y - 1.55, _cw - 4.0, 3.5, '#FFF8F0', label=None, lc=ORANGE_SOFT)
txt(_c0 + _cw / 2, _mid_y + 0.35, 'rank / step / stage unrolled into columns',
    fs=3.55, c=ORANGE, w='bold')
txt(_c0 + _cw / 2, _mid_y - 0.95, 'written to DuckDB trace (two-step window)',
    fs=3.25, c=GRAY, style='italic')
_rec_y = _oy + 3.85
arrow((_c0 + _cw / 2, _mid_y - 1.65), (_c0 + _cw / 2, _rec_y + 6.15),
      c=ORANGE, lw=LW_MID, ms=8, halo=True)
box(_c0 + 1.6, _rec_y, _cw - 3.2, 6.0, fc='#FBF6EF', ec=ORANGE, lw=LW_THIN, r=0.8)
txt(_c0 + _cw / 2, _rec_y + 4.55, 'captured record (S0):', fs=3.8, c=GRAY)
_pw = [('cksum', 4.6), ('param_name', 7.2), ('rank', 3.8), ('step', 3.8), ('stage', 4.6)]
_x = _c0 + 1.6 + ((_cw - 3.2) - (sum(w for _, w in _pw) + 0.55 * 4)) / 2
for _t, _w in _pw:
    pill(_x, _rec_y + 0.65, _w, 2.55, _t, 'white', ORANGE, fs=3.35)
    _x += _w + 0.55
...
# short bridges in gutters (inset so head stays inside gap, not past box edge)
_bridge_y = _oy + _oh * 0.52
arrow((_c0 + _cw - 0.1, _bridge_y), (_db0 + 0.15, _bridge_y),
      c=ORANGE, lw=LW_THICK, ms=9, halo=True)
arrow((_db0 + _dbw - 0.1, _bridge_y), (_v0 + 0.15, _bridge_y),
      c=ORANGE, lw=LW_THICK, ms=9, halo=True)
...
txt(_v0 + _vw / 2, _oy + 4.7, 'coarse$\\rightarrow$fine: checksum$\\rightarrow$module$\\rightarrow$param$\\rightarrow$bitwise',
    fs=3.15, c=GRAY)
# Verifier → violation (short gutter)
arrow((_v0 + _vw - 0.1, _bridge_y), (RX + 0.15, _bridge_y),
      c=RED_DEEP, lw=LW_THICK, ms=9, halo=True)
# Lib → Verifier: hub short drop → ❸ TOP; edge tag SQL (P0)
arrow((RX + 4.0, OFF_Y), (_v0 + _vw * 0.62, _ot), c=ORANGE, lw=LW_THICK, ms=10,
      cs='arc3,rad=-0.12', halo=True)
txt(104.2, (OFF_Y + _ot) / 2 + 0.35, 'SQL', fs=3.35, c=ORANGE, style='italic', z=6)
...
# viol → online output: under-lane lightened (P0 — less visual weight than online spine)
_uy = ON_Y - 1.0
_tip_y = ON_Y + 16.5
_rise_x = _OUTER_X + 0.9
_out_c = '#4A4A4A'
arrow((RX + 1.2, ON_Y + 1.1), (RX + 1.2, _uy), c=_out_c, lw=LW_MID, style='-',
      halo=True, z=3.4)
arrow((RX + 1.2, _uy), (_rise_x, _uy), c=_out_c, lw=LW_MID, style='-',
      halo=True, z=3.4)
arrow((_rise_x, _uy), (LX + LW + 0.2, _tip_y), c=_out_c, lw=LW_MID, ms=9,
      cs='arc3,rad=-0.14', halo=True, z=3.4)
...
fig.savefig('fig_overview_v32.pdf')
fig.savefig('/tmp/fig_overview_v32.png', dpi=240)
print('done v32')
