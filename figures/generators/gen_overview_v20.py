#!/usr/bin/env python3
"""v20 = v19 + module isolation blocks (QUITE nesting) (no label change).
Labels identical to v19. No Agents / 0-4 / 43k / fake hex.
"""
import matplotlib
matplotlib.use('Agg')
...
# isolate FSM + gates as one nested block (QUITE subsection)
dashed_group(28.35, 53.55, 76.3, 10.35, '#F0F4FA', label=None, lc=BLUE_SOFT)
txt(28.9, 62.15, 'five-stage FSM', fs=3.85, c=GRAY, ha='left')
...
cred = [(29.5, 'decisive counterexample gate'),
        ...
        (82.0, 'Conf$(c) \\geq 0.8$')]
for cx0, tt in cred:
    checkbox(cx0, 54.45, 1.25, GREEN)
    txt(cx0 + 1.95, 55.05, tt, fs=3.7, c='#0A6A68', ha='left')

# Accept → Library (from Accept card right edge — continuous offline story)
arrow((103.2, 71.7), (110.2, 76.8), c=BLUE_SOFT, lw=LW_THICK, ms=11, cs='arc3,rad=-0.16', halo=True)
txt(107.0, 79.9, 'accepted', fs=4.05, c=BLUE, style='italic', ha='right')
...
# Library → Online: two clear arcs (schema→Collector, compile→Verifier)
txt(105.5, 52.4, 'accepted rules determine trace schema (S0–S6)',
    fs=3.55, c=ORANGE, ha='right')
# schema → Collector top (lands on ❷, not into Verifier)
arrow((108.5, 51.5), (55.0, 47.8), c=ORANGE, lw=LW_MID, ms=10, cs='arc3,rad=0.08', halo=True)
# compile → Verifier right edge (keeps clear of violation panel)
arrow((120.5, 46.3), (106.6, 33.5), c=ORANGE, lw=LW_THICK, ms=11, cs='arc3,rad=0.22', halo=True)
...
# DuckDB bridge — isolated store block between ❷ and ❸
dashed_group(63.6, 18.6, 11.6, 20.2, '#FFF4E8', label=None, lc=ORANGE)
...
# online spine: Collector → DuckDB → Verifier (one continuous orange chain)
arrow((63.5, 31.8), (64.95, 31.8), c=ORANGE, lw=LW_THICK, ms=8, halo=True)
arrow((73.85, 31.8), (75.45, 31.8), c=ORANGE, lw=LW_THICK, ms=8, halo=True)
...
fig.savefig('fig_overview_v20.pdf')
fig.savefig('/tmp/fig_overview_v20.png', dpi=240)
print('done v20')
