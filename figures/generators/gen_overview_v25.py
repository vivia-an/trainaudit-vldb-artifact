#!/usr/bin/env python3
"""v25 = v24 + denser QUITE packing (no label change).
Compact lanes/Miner/online internals; lighter under-arc; tighter title air.
Labels identical to v24/v22. No Agents / 0-4 / 43k / fake hex.
"""
import matplotlib
matplotlib.use('Agg')
...
arrow((RX, OFF_Y + 1.5), (55.0, ON_Y + ON_H - 0.5), c=ORANGE, lw=LW_MID, ms=9,
      cs='arc3,rad=0.10', halo=True)
arrow((_rc - 2.0, OFF_Y), (106.6, ON_Y + 17.2), c=ORANGE, lw=LW_THICK, ms=10,
      cs='arc3,rad=0.16', halo=True)

# ================= Data Collector (online row) — denser =================
_oy, _oh = ON_Y, ON_H
_ot = _oy + _oh
shadow(27, _oy, 36.5, _oh)
box(27, _oy, 36.5, _oh, fc=CARD, ec='#111111', lw=LW_MID + 0.15, r=1.15)
badge(30.6, _ot - 2.2, '2')
txt(33.6, _ot - 2.2, 'Data Collector', fs=6.2, w='bold', c=ORANGE, ha='left')
chip(55.0, _ot - 2.2, 1.35, ORANGE)
icon_funnel(55.0, _ot - 2.2, 0.76, 'white')
txt(28.4, _ot - 4.55, 'no user code changes · adapter 30–150 LoC', fs=3.65,
...
for i, a in enumerate(anchors):
    ...
    txt(x + w / 2 + 0.15, _chev_mid, a, fs=3.35, family='monospace', w='bold', z=5)
txt(45.2, _chev_mid - 4.45, '+ auxiliary taps (ckpt · all_reduce · snapshot) = 8 hookpoints',
    fs=3.45, c=GRAY)
txt(45.2, _chev_mid - 6.55, 'GPU-side scalar reductions (checksums only when required)', fs=3.55)
_rec_y = _oy + 2.15
box(28.8, _rec_y, 33, 6.35, fc='#FBF6EF', ec=ORANGE, lw=LW_THIN, r=0.8)
txt(45.3, _rec_y + 4.85, 'captured record (S0):', fs=3.8, c=GRAY)
_pw = [('cksum', 4.9), ('param_name', 7.7), ('rank', 4.1), ('step', 4.1), ('stage', 4.9)]
_x = 28.8 + (33 - (sum(w for _, w in _pw) + 0.65 * 4)) / 2
for _t, _w in _pw:
    pill(_x, _rec_y + 0.7, _w, 2.7, _t, 'white', ORANGE, fs=3.45)
    _x += _w + 0.65

# DuckDB bridge — height matches mid-lane content
_db_y = _oy + 3.6
dashed_group(64.0, _db_y, 10.8, 19.4, '#FFF4E8', label=None, lc=ORANGE)
cyl(65.2, _db_y + 7.2, 8.4, 10.6, fc='#F5E6D0', ec=ORANGE)
for _yy in (_db_y + 10.6, _db_y + 13.3):
    ax.plot([66.0, 72.8], [_yy, _yy], color=ORANGE, lw=LW_HAIR, alpha=0.45, zorder=4)
txt(69.4, _db_y + 4.7, 'DuckDB\ntrace DB', fs=4.05, w='bold', c=ORANGE, ls=1.18)
txt(69.4, _db_y + 1.05, 'two-step\nsliding window', fs=3.3, c=GRAY, ls=1.18)
_bridge_y = _oy + _oh * 0.50
...
box(75.5, _oy, 31.0, _oh, fc=CARD, ec='#111111', lw=LW_MID + 0.15, r=1.15)
badge(79.1, _ot - 2.2, '3')
txt(82.1, _ot - 2.2, 'Verifier', fs=6.2, w='bold', c=ORANGE, ha='left')
chip(92.2, _ot - 2.2, 1.35, ORANGE)
icon_verify(92.2, _ot - 2.2, 0.76, 'white')
txt(76.7, _ot - 4.55, 'deterministic SQL checking — no LLM', fs=3.65,
    style='italic', c=GRAY, ha='left')
_vq_y, _vq_h = _oy + 10.2, 14.85
box(76.8, _vq_y, 13.6, _vq_h, fc=MINT, ec='#7A9E72', lw=LW_HAIR, r=0.85, ls=(0, (2.6, 1.8)))
box(77.35, _vq_y + 0.5, 12.5, _vq_h - 1.0, fc='#FFFFFF', ec='#B8C8B4', lw=LW_HAIR, r=0.65)
txt(83.6, _vq_y + _vq_h - 1.4, 'at job start', fs=4.2, w='bold')
ax.plot([78.4, 88.8], [_vq_y + _vq_h - 2.6, _vq_y + _vq_h - 2.6], color=GRAY_LINE, lw=LW_HAIR, zorder=5)
txt(83.6, _vq_y + 5.85, 'extract topology $\\tau$\ntopology-aware\npruning · lineage:\nreplicated vs.\nsharded', fs=3.55, ls=1.32)
box(91.4, _vq_y, 14.0, _vq_h, fc='#1E242C', ec='#111111', lw=LW_THIN, r=0.85)
txt(98.4, _vq_y + _vq_h - 1.4, 'per step, per rule', fs=4.15, w='bold', c='#9DB8E8')
ax.plot([92.7, 104.1], [_vq_y + _vq_h - 2.6, _vq_y + _vq_h - 2.6], color='#7A8A9A', lw=LW_HAIR, zorder=5)
_KW, _TX, _PI = '#61AFEF', '#E8EBF0', '#E5C07B'
_sql_y = _vq_y + _vq_h - 4.4
txt(92.7, _sql_y, 'WHERE', fs=3.55, ha='left', family='monospace', c=_KW, w='bold')
txt(96.55, _sql_y, '$\\pi_{topo}\\wedge\\pi_{precond}$', fs=3.4, ha='left', c=_PI, w='bold')
txt(92.7, _sql_y - 2.35, 'GROUP BY', fs=3.55, ha='left', family='monospace', c=_KW, w='bold')
txt(93.3, _sql_y - 4.35, 'param, step', fs=3.55, ha='left', family='monospace', c=_TX, w='bold')
txt(92.7, _sql_y - 6.55, 'HAVING', fs=3.55, ha='left', family='monospace', c=_KW, w='bold')
txt(96.7, _sql_y - 6.55, '$\\pi_{schema}$', fs=3.4, ha='left', c=_PI, w='bold')
txt(93.3, _sql_y - 8.45, 'violated', fs=3.45, ha='left', family='monospace', c=_TX, w='bold')
txt(91.0, _oy + 7.7, 'empty result = holds · non-empty IS the violation set',
    fs=3.45, style='italic')
txt(91.0, _oy + 4.7, 'coarse$\\rightarrow$fine: checksum$\\rightarrow$module$\\rightarrow$param$\\rightarrow$bitwise',
    fs=3.25, c=GRAY)
arrow((106.5, _bridge_y), (108.4, _bridge_y), c=RED_DEEP, lw=LW_THICK, ms=10, halo=True)

# ================= Violation panel (= ON block y/h) =================
shadow(RX, ON_Y, RW, ON_H)
box(RX, ON_Y, RW, ON_H, fc=VIOL_BG, ec=RED, lw=LW_MID, r=1.05, ls=(0, (3.0, 2.0)))
txt(_rc, ON_Y + ON_H - 1.6, 'violation report', fs=5.05, w='bold', c=RED)
ax.plot([_rc - 8.2, _rc + 8.2], [ON_Y + ON_H - 2.85, ON_Y + ON_H - 2.85],
...
for _r, (_p, _rk, _ck, _bad) in enumerate(_rows):
    ...
    for _v, (_h, _wd) in zip((_p, _rk, _ck), _cols):
        ...
        _cx += _wd
txt(_rc, ON_Y + 13.2, '$\\mathbf{\\times}$ divergent checksums', fs=3.65, c=RED)
for _k, _it in enumerate(['violating constraint', 'affected parameter',
                          'divergent ranks', 'first violating step']):
    _yy = ON_Y + 10.8 - _k * 2.5
    checkbox(RX + 3.0, _yy - 0.5, 1.15, RED)
    txt(RX + 5.05, _yy, _it, fs=3.7, ha='left')
# viol → online output: stay in bottom pad, lighter stroke (less clutter)
arrow((RX, ON_Y + 0.85), (LX + LW, ON_Y + 5.2), c=DARK, lw=LW_MID + 0.2, ms=10,
      cs='arc3,rad=0.26', halo=True)
...
fig.savefig('fig_overview_v25.pdf')
fig.savefig('/tmp/fig_overview_v25.png', dpi=240)
print('done v25')
