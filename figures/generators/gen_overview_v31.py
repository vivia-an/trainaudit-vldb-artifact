#!/usr/bin/env python3
"""v31 = v30 + Data Collector mid-band (scheme C, paper-true).
rank/step/stage unrolled into columns + flow cue; S0 raised.
No Agents / 0-4 / 43k / fake hex.
"""
import matplotlib
matplotlib.use('Agg')
...
# Lib → Collector (schema); Lib → Verifier drawn after ❸ exists (top-edge tip)
...
# Lib → Verifier: hub gutter → tip on ❸ TOP edge (clear of SQL / mint cards)
...
# viol → online output: short under-lane; rise starts at outer left (no bay overflow)
_uy = ON_Y - 1.0
_tip_y = ON_Y + 16.5
_rise_x = _OUTER_X + 0.9            # keep horizontal inside outer frame
arrow((RX + 1.2, ON_Y + 1.1), (RX + 1.2, _uy), c=DARK, lw=LW_THICK, style='-',
      halo=True, z=3.4)
arrow((RX + 1.2, _uy), (_rise_x, _uy), c=DARK, lw=LW_THICK, style='-',
      halo=True, z=3.4)
arrow((_rise_x, _uy), (LX + LW + 0.2, _tip_y), c=DARK, lw=LW_THICK, ms=10,
...
fig.savefig('fig_overview_v31.pdf')
fig.savefig('/tmp/fig_overview_v31.png', dpi=240)
print('done v31')
