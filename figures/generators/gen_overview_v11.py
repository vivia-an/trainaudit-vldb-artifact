#!/usr/bin/env python3
"""v11 = v10 visual genome minus bloodline toxins.
Components only 1/2/3 (Miner/Collector/Verifier). No LLM-agent legend.
P3 output not fused with 43k GPU-hours. Checksums abstracted.
"""
import matplotlib
matplotlib.use('Agg')
...
txt(84, 91.3, 'TrainAudit', fs=7.6, w='bold', c=BLUE)
ax.plot([40, 76], [91.3, 91.3], color=BLUE, lw=0.5, zorder=4)
ax.plot([92, 128], [91.3, 91.3], color=BLUE, lw=0.5, zorder=4)
...
icon_brain(36.8, 75.6, 0.85, '#8a7aa8')
...
# ================= legend (arrow types only; no agent row) =================
txt(25.5, 4.0, 'Legend', fs=4.3, w='bold', ha='left')
ax.plot([35, 41], [4.0, 4.0], color=BLUE, lw=1.4)
txt(42.5, 4.0, 'offline mining', fs=3.9, ha='left')
ax.plot([60, 66], [4.0, 4.0], color=ORANGE, lw=1.4)
txt(67.5, 4.0, 'online checking', fs=3.9, ha='left')
ax.plot([90, 96], [4.0, 4.0], color=RED, lw=1.2, ls=(0, (3, 2)))
txt(97.5, 4.0, 'reject / violation', fs=3.9, ha='left')
txt(142.5, 4.0, 'LLM only inside offline Miner · online path: no LLM',
    fs=3.8, style='italic', c=GRAY, ha='right')

fig.savefig('fig_overview_v11.pdf')
fig.savefig('/tmp/fig_overview_v11.png', dpi=220)
print('done')
