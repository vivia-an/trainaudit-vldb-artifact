#!/usr/bin/env python3
"""v16 = v15 + QUITE size/padding/desat/sharp DNA.
Labels identical to v15. No Agents / 0-4 / 43k / fake hex.
Focus: box-internal pad, type hierarchy, pale washes, flat (no muddy shadow).
"""
import matplotlib
matplotlib.use('Agg')
...
# QUITE desaturated depth (pale zone + sharp ink)
BLUE = '#3A6EA5'
BLUE_SOFT = '#7A9CC6'
BLUE_DEEP = '#1F3A56'
BLUE_BG = '#F3F6FA'       # near-white blue wash
ORANGE = '#C45E28'
ORANGE_BG = '#FCF8F3'
ORANGE_SOFT = '#E2B08A'
RED = '#B25A56'
...
GREEN = '#2AA6A3'
GREEN_BG = '#EEF7F6'
DARK = '#1A1A1A'          # sharp ink (QUITE black text)
GRAY = '#5A5A5A'
GRAY_LINE = '#A8A8A8'
CARD = '#FFFFFF'          # pure white nested cards
HUB_BG = '#F8F6F2'
VIOL_BG = '#FBF5F4'
LAVENDER = '#EDE6F2'      # softer than v15
PURPLE = '#8E759C'
MINT = '#E6F0E4'
PEACH = '#F6E6E3'
CREAM = '#FFF9EC'
# stroke ladder
LW_HAIR, LW_THIN, LW_MID, LW_THICK = 0.28, 0.55, 0.85, 1.45
...
txt(84, 94.8, 'TrainAudit', fs=8.4, w='bold', c=BLUE)
...
# lane headers — soft pill, more pad around title
ax.add_patch(FancyBboxPatch((27.2, 90.4), 78.6, 2.5,
             boxstyle='round,pad=0,rounding_size=0.55',
             fc='#E8EEF6', ec='none', zorder=2))
txt(28.0, 91.65, 'OFFLINE — mine once per framework', fs=5.85, w='bold', c=BLUE, ha='left')
ax.add_patch(FancyBboxPatch((27.2, 49.4), 78.6, 2.5,
             boxstyle='round,pad=0,rounding_size=0.55',
             fc='#F5E9DC', ec='none', zorder=2))
txt(28.0, 50.65, 'ONLINE — every training job · fully deterministic · no LLM',
    fs=5.85, w='bold', c=ORANGE, ha='left')
...
def folded_doc(x, y, w, h, lines, title=None, title_c=DARK, ec=DARK, fc='#fafafa', hl=None):
    ...
    for i, ln in enumerate(lines):
        ...
        cc = (hl or {}).get(i, DARK)
        txt(x + 1.15, y0 - i * 1.95, ln, fs=3.25, family=fam, ha='left',
            c=cc, w='bold' if i in (hl or {}) else 'normal', z=5, ls=1.3)
...
box(29.0, 76.55, 12.8, 2.15, fc='#F7F2FA', ec=PURPLE, lw=LW_HAIR, r=0.4, ls=(0, (1.8, 1.2)))
txt(35.4, 77.6, 'instantiate', fs=3.65, w='bold', c=PURPLE)
box(67.6, 76.55, 16.8, 2.15, fc='#F2F7F0', ec='#7A9E72', lw=LW_HAIR, r=0.4, ls=(0, (1.8, 1.2)))
txt(76.0, 77.6, 'verify (decisive)', fs=3.65, w='bold', c='#4F7A4F')
...
box(110.5, 71.7, 28.5, 4.8, fc='#2F343C', ec='none', r=1.0, z=3)
ax.add_patch(Rectangle((110.5, 71.7), 28.5, 2.2, fc='#2F343C', ec='none', zorder=3))
txt(124.7, 75.0, 'P3 · cross-rank replication', fs=4.75, w='bold', c='white', z=5)
txt(124.7, 72.75, '(SwitchMLP router sync)', fs=3.7, c='#C8CDD8', z=5)
...
anchors = ['before-\nforward', 'after-\nforward', 'main-grad-\nin-backward',
           'after-\nbackward', 'before-\noptimizer']
# desaturated chevron ladder (less muddy orange)
tints = ['#F8E8D4', '#F3DCC0', '#EDD0AC', '#E6C498', '#DEB884']
for i, a in enumerate(anchors):
    x = 28.3 + i * 6.85
    w, n = 7.4, 1.2
    pts = [(x, 36.85), (x + w - n, 36.85), (x + w, 39.7), (x + w - n, 42.55),
           (x, 42.55), (x + n, 39.7)]
    ax.add_patch(Polygon(pts, closed=True, fc=tints[i], ec='#C49A6C', lw=LW_HAIR, zorder=3))
    txt(x + w / 2 + 0.15, 39.7, a, fs=3.3, family='monospace', z=5)
...
box(91.4, 27.0, 14.0, 14.9, fc='#2F343C', ec='#222222', lw=LW_THIN, r=0.9)
txt(98.4, 40.35, 'per step, per rule', fs=4.1, w='bold', c='#A8C0E8')
ax.plot([92.7, 104.1], [39.0, 39.0], color='#7A8A9A', lw=LW_HAIR, zorder=5)
_KW, _TX, _PI = '#6BB0F0', '#E0E3E8', '#E8C48A'
txt(92.7, 37.05, 'WHERE', fs=3.45, ha='left', family='monospace', c=_KW)
txt(96.55, 37.05, '$\\pi_{topo}\\wedge\\pi_{precond}$', fs=3.3, ha='left', c=_PI)
txt(92.7, 34.4, 'GROUP BY', fs=3.45, ha='left', family='monospace', c=_KW)
txt(93.3, 32.15, 'param, step', fs=3.45, ha='left', family='monospace', c=_TX)
txt(92.7, 29.75, 'HAVING', fs=3.45, ha='left', family='monospace', c=_KW)
txt(96.7, 29.75, '$\\pi_{schema}$', fs=3.3, ha='left', c=_PI)
txt(93.3, 27.9, 'violated', fs=3.45, ha='left', family='monospace', c=_TX)
...
fig.savefig('fig_overview_v16.pdf')
fig.savefig('/tmp/fig_overview_v16.png', dpi=240)
print('done v16')
