#!/usr/bin/env python3
"""v26 = v25 + compact violation-report stack (no label change).
Table → ×label → checklist packed top-down (no mid-panel air).
Labels identical to v25/v22. No Agents / 0-4 / 43k / fake hex.
"""
import matplotlib
matplotlib.use('Agg')
...
# exit just under packed stack (eye follows content → online output; no orphan bottom stub)
_exit_y = _chk0 - (len(_items) - 1) * _pitch - 1.35
arrow((RX, max(ON_Y + 0.8, _exit_y)), (LX + LW, ON_Y + 5.2), c=DARK,
      lw=LW_MID + 0.2, ms=10, cs='arc3,rad=0.22', halo=True)
...
fig.savefig('fig_overview_v26.pdf')
fig.savefig('/tmp/fig_overview_v26.png', dpi=240)
print('done v26')
