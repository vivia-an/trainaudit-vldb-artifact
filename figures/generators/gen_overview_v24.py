#!/usr/bin/env python3
"""v24 = v23 + title/bottom/arrow polish (no label change).
TrainAudit back inside frame; bottom pad tightened; viol→output under-arc.
Labels identical to v23/v22. No Agents / 0-4 / 43k / fake hex.
"""
import matplotlib
matplotlib.use('Agg')
...
# Twin lanes share H; band sits in the same inter-lane strip on both sides.
BAND_X, BAND_W, BAND_H = 27.2, 78.6, 2.4
BAND_INSET = 0.45
ON_Y, ON_H = 16.4, 35.0            # whole online lane nudged up
BAND_ON_Y = ON_Y + ON_H + BAND_INSET
OFF_Y = BAND_ON_Y + BAND_H + BAND_INSET   # = online-top strip → offline block
OFF_H = ON_H                       # equal lane height (true left/right mirror)
BAND_OFF_Y = OFF_Y + OFF_H + BAND_INSET
LX, LW = 0.8, 23.0                 # left bay
RX, RW = 108.5, 32.5               # right bay
CX = LX + LW / 2                   # left-bay center x

# ================= outer container =================
# bottom pad tight (content↑); top reserves air so TrainAudit sits INSIDE frame
_BOTTOM_PAD = 2.35
_TITLE_AIR = 2.45          # band-top → title center (v22-like inside air)
_TOP_PAD = 2.05            # title center → outer top
...
shadow(25, _outer_y, 118, _outer_h, r=1.6)
box(25, _outer_y, 118, _outer_h, fc='#FFFFFF', ec='#111111', lw=LW_THICK, r=1.4, z=1.8)
# lane washes aligned to OFF/ON blocks (+ band)
box(26.4, OFF_Y - 0.55, 80.2, OFF_H + BAND_H + BAND_INSET + 1.1, fc=BLUE_BG, ec='none', r=1.15, z=1.85)
box(26.4, ON_Y - 0.55, 80.2, ON_H + BAND_H + BAND_INSET + 1.1, fc=ORANGE_BG, ec='none', r=1.15, z=1.85)
box(RX - 0.3, ON_Y - 0.55, RW + 0.6, OFF_Y + OFF_H - ON_Y + 1.1, fc=HUB_BG, ec='none', r=1.15, z=1.85)
txt(84, _title_y, 'TrainAudit', fs=8.8, w='bold', c=BLUE_DEEP)
ax.plot([37.2, 73.8], [_title_y, _title_y], color=BLUE_SOFT, lw=LW_HAIR, zorder=4)
ax.plot([94.2, 130.8], [_title_y, _title_y], color=BLUE_SOFT, lw=LW_HAIR, zorder=4)

# twin lane headers — identical geometry, mirrored placement
...
txt(BAND_X + 0.8, BAND_OFF_Y + BAND_H / 2, 'OFFLINE — mine once per framework',
    fs=6.15, w='bold', c=BLUE_DEEP, ha='left')
...
txt(LX + 1.2, OFF_Y + OFF_H - 1.7, 'offline inputs', fs=5.25, w='bold', c=BLUE, ha='left')
_doc_w = LW - 2.2
folded_doc(LX + 1.1, OFF_Y + 17.6, _doc_w, 15.2,
           lines=['template P3:',
                  '  pi_topo: TP > 1',
...
txt(CX, OFF_Y + 16.05, '16 templates · 13 fault classes', fs=3.35, c=GRAY)
folded_doc(LX + 1.1, OFF_Y + 1.5, _doc_w, 13.2,
           lines=['Megatron-LM',
                  'DeepSpeed',
                  'OLMo / OLMo-core',
...
txt(CX, OFF_Y + 0.5, 'once per framework', fs=3.35, style='italic', c=GRAY)
...
# ================= Invariant Miner (offline row) — y tied to OFF_* =================
_my, _mh = OFF_Y, OFF_H
shadow(27, _my, 79, _mh)
box(27, _my, 79, _mh, fc=CARD, ec='#111111', lw=LW_MID + 0.15, r=1.2)
_mt = _my + _mh
badge(30.6, _mt - 2.6, '1')
txt(33.6, _mt - 2.6, 'Invariant Miner', fs=6.6, w='bold', c=BLUE_DEEP, ha='left')
chip(53.4, _mt - 2.6, 1.55, BLUE)
icon_gear(53.4, _mt - 2.6, 0.9, 'white')
txt(104.8, _mt - 2.6, 'LLM-instantiated candidates, untrusted until verified',
    fs=3.7, style='italic', c=GRAY, ha='right')

# compose row — even pitch; semantic icons
_comp_y = _mt - 9.35
dashed_group(28.3, _comp_y, 76.4, 5.2, '#F2F6FC', label=None, lc=BLUE_SOFT)
_plus_w, _plus_gap = 15.8, 3.35
_plus_x0 = 29.0
plus_cards = [
...
for i, (ic, lb) in enumerate(plus_cards):
    ...
    box(px, _comp_y + 0.35, _plus_w, 4.55, fc='#FFFFFF', ec=BLUE_SOFT, lw=LW_THIN, r=0.75)
    ic(px + 2.4, _comp_y + 2.6, 0.72, BLUE)
    txt(px + 5.05, _comp_y + 2.6, lb, fs=3.4, ls=1.32, ha='left')
for i in range(3):
    gx = _plus_x0 + (i + 1) * _plus_w + i * _plus_gap + _plus_gap / 2
    txt(gx, _comp_y + 2.6, '+', fs=6.2, w='bold', c=GRAY_LINE)
arrow((66.2, _comp_y + 0.2), (66.2, _comp_y - 0.85), c=BLUE_SOFT, lw=LW_THICK, ms=9, halo=True)

_step_y, _step_h = _my + 12.85, 9.2
dashed_group(28.2, _step_y - 0.35, 38.2, _step_h + 3.55, LAVENDER, label=None, lc=PURPLE)
dashed_group(66.8, _step_y - 0.35, 37.8, _step_h + 3.55, MINT, label=None, lc='#7A9E72')
box(29.0, _step_y + _step_h + 0.55, 12.8, 2.0, fc='#D9C8E4', ec=PURPLE, lw=LW_HAIR, r=0.4, ls=(0, (1.8, 1.2)))
txt(35.4, _step_y + _step_h + 1.55, 'instantiate', fs=3.9, w='bold', c='#6B4F7A')
box(67.6, _step_y + _step_h + 0.55, 16.8, 2.0, fc='#BFD9B8', ec='#5B8A5B', lw=LW_HAIR, r=0.4, ls=(0, (1.8, 1.2)))
txt(76.0, _step_y + _step_h + 1.55, 'verify (decisive)', fs=3.9, w='bold', c='#3D6B3D')
icon_brain(42.4, _step_y + _step_h + 1.55, 0.68, PURPLE)
...
_title_yy = _step_y + _step_h - 1.9
for x, name, sub, ec, lw, fc in steps:
    box(x, _step_y, 16.3, _step_h, fc=fc, ec=ec, lw=(LW_MID if ec is RED else LW_THIN), r=0.9)
    txt(x + 9.5, _title_yy, name, fs=5.45, w='bold', c=ec)
    ax.plot([x + 2.4, x + 14.0], [_title_yy - 1.15, _title_yy - 1.15], color=ec, lw=LW_HAIR, alpha=0.55, zorder=5)
    txt(x + 8.15, _step_y + 2.35, sub, fs=3.55, ls=1.26)
    if name == 'Scope':
        icon_target(x + 3.0, _title_yy, 0.8, DARK)
    elif name == 'Ground':
        icon_anchor(x + 3.0, _title_yy, 0.78, DARK)
    elif name == 'Accept':
        icon_checkc(x + 3.15, _title_yy, 0.8, DARK)
    elif name == 'Construct':
        chip(x + 2.7, _title_yy, 1.15, RED)
        icon_bolt(x + 2.7, _title_yy + 0.05, 0.72, 'white')
_mid_y = _step_y + _step_h / 2
for x0 in (45.7, 64.95, 84.25):
    arrow((x0, _mid_y), (x0 + 2.35, _mid_y), c=BLUE_SOFT, lw=LW_MID, ms=7)

# reject loop — between step cards and FSM
_rej_y = _step_y - 1.15
arrow((95.0, _step_y - 0.15), (95.0, _rej_y), c=RED_DEEP, lw=LW_THIN, ls=(0, (2.4, 1.5)), style='-')
arrow((95.0, _rej_y), (37.3, _rej_y), c=RED_DEEP, lw=LW_THIN, ls=(0, (2.4, 1.5)), style='-')
arrow((37.3, _rej_y), (37.3, _step_y - 0.2), c=RED_DEEP, lw=LW_THIN, ls=(0, (2.4, 1.5)))
red_x(99.3, _rej_y, 0.5)
txt(66.0, _rej_y - 0.85, 'reject $\\rightarrow$ next iteration', fs=3.55, c=RED, style='italic')

# FSM block
_fsm_box_y, _fsm_box_h = _my + 0.25, 9.6
dashed_group(28.35, _fsm_box_y, 76.3, _fsm_box_h, '#F0F4FA', label=None, lc=BLUE_SOFT)
txt(28.9, _fsm_box_y + _fsm_box_h - 1.55, 'five-stage FSM', fs=3.75, c=GRAY, ha='left')
...
_fsm_r, _fsm_y, _fsm_dx = 1.75, _fsm_box_y + 5.85, 9.05
for k, (sn, lb, cc) in enumerate(fsm):
    cx = 40.6 + k * _fsm_dx
    ax.add_patch(Circle((cx, _fsm_y), _fsm_r, fc=cc, ec='#222222', lw=LW_HAIR, zorder=5))
    txt(cx, _fsm_y - 0.05, sn, fs=5.1, c='white', w='bold', z=6)
    txt(cx, _fsm_y - 2.65, lb, fs=3.05, c=DARK)
    if k < 4:
        arrow((cx + _fsm_r + 0.22, _fsm_y), (cx + _fsm_dx - _fsm_r - 0.22, _fsm_y),
...
cred = [(29.5, 'decisive counterexample gate'),
        ...
        (83.5, 'Conf$(c) \\geq 0.8$')]
for cx0, tt in cred:
    checkbox(cx0, _my + 0.95, 1.15, GREEN)
    txt(cx0 + 1.9, _my + 1.5, tt, fs=3.45, c='#0A6A68', ha='left')

arrow((103.2, _mid_y), (110.2, _mt - 11.0), c=BLUE_SOFT, lw=LW_THICK, ms=11, cs='arc3,rad=-0.16', halo=True)
txt(106.2, _mt - 7.2, 'accepted', fs=4.0, c=BLUE, style='italic', ha='right')

# ================= Verified Constraint Library (= OFF block y/h, mirrors left) =================
_rc = RX + RW / 2
shadow(RX, OFF_Y, RW, OFF_H)
box(RX, OFF_Y, RW, OFF_H, fc=CARD, ec='#111111', lw=LW_MID + 0.15, r=1.2)
cyl(_rc - 4.25, OFF_Y + OFF_H - 6.2, 8.5, 4.6, fc='#EDE6D8', ec=DARK)
txt(_rc, OFF_Y + OFF_H - 8.6, 'Verified Constraint Library $\\mathcal{P}$', fs=4.85, w='bold', c=BLUE_DEEP)
txt(_rc, OFF_Y + OFF_H - 10.6, 'guarded relational constraints', fs=3.7, style='italic', c=GRAY)
_p3y, _p3h = OFF_Y + 8.6, 16.8
shadow(RX + 2.0, _p3y, RW - 4.0, _p3h, r=1.0)
box(RX + 2.0, _p3y, RW - 4.0, _p3h, fc='#FFFFFF', ec='#222222', lw=LW_THIN, r=1.0)
box(RX + 2.0, _p3y + _p3h - 4.5, RW - 4.0, 4.5, fc='#1E242C', ec='none', r=1.0, z=3)
ax.add_patch(Rectangle((RX + 2.0, _p3y + _p3h - 4.5), RW - 4.0, 2.0, fc='#1E242C', ec='none', zorder=3))
txt(_rc, _p3y + _p3h - 1.5, 'P3 · cross-rank replication', fs=4.6, w='bold', c='white', z=5)
txt(_rc, _p3y + _p3h - 3.4, '(SwitchMLP router sync)', fs=3.55, w='bold', c='#B8C4D8', z=5)
txt(RX + 3.5, _p3y + 5.6, '$\\pi_{topo}$:  TP > 1\n$\\pi_{precond}$:  tpl = False\n   (not tensor-parallel sharded)\n$\\pi_{schema}$:  replicas share\n   checksum',
    fs=3.85, ha='left', ls=1.32)
txt(_rc, OFF_Y + 6.3, 'compiled to parameterized SQL at job start', fs=3.3, c=ORANGE, w='bold')
txt(_rc, OFF_Y + 4.7, 'accepted rules determine trace schema (S0–S6)',
    fs=3.2, c=ORANGE, style='italic')
pill(RX + 2.0, OFF_Y + 1.2, RW - 4.0, 2.8, '', GREEN_BG, GREEN)
txt(_rc, OFF_Y + 2.6, '$\\checkmark$  0 false positives over 764 clean evals', fs=3.5, c='#0A6A68', z=6)
arrow((RX, OFF_Y + 1.8), (55.0, ON_Y + ON_H - 0.6), c=ORANGE, lw=LW_MID, ms=10,
      cs='arc3,rad=0.10', halo=True)
arrow((_rc - 2.0, OFF_Y), (106.6, ON_Y + 18.5), c=ORANGE, lw=LW_THICK, ms=11,
      cs='arc3,rad=0.18', halo=True)

# ================= Data Collector (online row) — y tied to ON_* =================
_oy, _oh = ON_Y, ON_H
_ot = _oy + _oh
shadow(27, _oy, 36.5, _oh)
box(27, _oy, 36.5, _oh, fc=CARD, ec='#111111', lw=LW_MID + 0.15, r=1.2)
badge(30.6, _ot - 2.5, '2')
txt(33.6, _ot - 2.5, 'Data Collector', fs=6.45, w='bold', c=ORANGE, ha='left')
chip(55.2, _ot - 2.5, 1.45, ORANGE)
icon_funnel(55.2, _ot - 2.5, 0.82, 'white')
txt(28.4, _ot - 5.0, 'no user code changes · adapter 30–150 LoC', fs=3.8,
...
for i, a in enumerate(anchors):
    x = 28.3 + i * 6.85
    w, n = 7.4, 1.2
    pts = [(x, _chev_mid - 2.85), (x + w - n, _chev_mid - 2.85), (x + w, _chev_mid),
           (x + w - n, _chev_mid + 2.85), (x, _chev_mid + 2.85), (x + n, _chev_mid)]
    ax.add_patch(Polygon(pts, closed=True, fc=tints[i], ec=ORANGE, lw=LW_HAIR, zorder=3))
    txt(x + w / 2 + 0.15, _chev_mid, a, fs=3.5, family='monospace', w='bold', z=5)
txt(45.2, _chev_mid - 5.0, '+ auxiliary taps (ckpt · all_reduce · snapshot) = 8 hookpoints',
    fs=3.6, c=GRAY)
txt(45.2, _chev_mid - 7.4, 'GPU-side scalar reductions (checksums only when required)', fs=3.7)
_rec_y = _oy + 2.5
box(28.8, _rec_y, 33, 7.0, fc='#FBF6EF', ec=ORANGE, lw=LW_THIN, r=0.85)
txt(45.3, _rec_y + 5.4, 'captured record (S0):', fs=3.95, c=GRAY)
_pw = [('cksum', 4.9), ('param_name', 7.7), ('rank', 4.1), ('step', 4.1), ('stage', 4.9)]
_x = 28.8 + (33 - (sum(w for _, w in _pw) + 0.65 * 4)) / 2
for _t, _w in _pw:
    pill(_x, _rec_y + 0.85, _w, 2.9, _t, 'white', ORANGE, fs=3.55)
    _x += _w + 0.65

# DuckDB bridge
_db_y = _oy + 4.2
dashed_group(64.0, _db_y, 10.8, 20.8, '#FFF4E8', label=None, lc=ORANGE)
cyl(65.2, _db_y + 7.8, 8.4, 11.5, fc='#F5E6D0', ec=ORANGE)
for _yy in (_db_y + 11.5, _db_y + 14.4):
    ax.plot([66.0, 72.8], [_yy, _yy], color=ORANGE, lw=LW_HAIR, alpha=0.45, zorder=4)
txt(69.4, _db_y + 5.2, 'DuckDB\ntrace DB', fs=4.2, w='bold', c=ORANGE, ls=1.2)
txt(69.4, _db_y + 1.2, 'two-step\nsliding window', fs=3.45, c=GRAY, ls=1.2)
_bridge_y = _oy + _oh * 0.52
arrow((63.5, _bridge_y), (63.95, _bridge_y), c=ORANGE, lw=LW_THICK, ms=8, halo=True)
arrow((74.85, _bridge_y), (75.45, _bridge_y), c=ORANGE, lw=LW_THICK, ms=8, halo=True)

# ================= Verifier =================
shadow(75.5, _oy, 31.0, _oh)
box(75.5, _oy, 31.0, _oh, fc=CARD, ec='#111111', lw=LW_MID + 0.15, r=1.2)
badge(79.1, _ot - 2.5, '3')
txt(82.1, _ot - 2.5, 'Verifier', fs=6.45, w='bold', c=ORANGE, ha='left')
chip(92.4, _ot - 2.5, 1.45, ORANGE)
icon_verify(92.4, _ot - 2.5, 0.82, 'white')
txt(76.7, _ot - 5.0, 'deterministic SQL checking — no LLM', fs=3.8,
    style='italic', c=GRAY, ha='left')
_vq_y, _vq_h = _oy + 11.2, 15.6
box(76.8, _vq_y, 13.6, _vq_h, fc=MINT, ec='#7A9E72', lw=LW_HAIR, r=0.9, ls=(0, (2.6, 1.8)))
box(77.35, _vq_y + 0.55, 12.5, _vq_h - 1.1, fc='#FFFFFF', ec='#B8C8B4', lw=LW_HAIR, r=0.7)
txt(83.6, _vq_y + _vq_h - 1.55, 'at job start', fs=4.4, w='bold')
ax.plot([78.4, 88.8], [_vq_y + _vq_h - 2.85, _vq_y + _vq_h - 2.85], color=GRAY_LINE, lw=LW_HAIR, zorder=5)
txt(83.6, _vq_y + 6.2, 'extract topology $\\tau$\ntopology-aware\npruning · lineage:\nreplicated vs.\nsharded', fs=3.7, ls=1.38)
box(91.4, _vq_y, 14.0, _vq_h, fc='#1E242C', ec='#111111', lw=LW_THIN, r=0.9)
txt(98.4, _vq_y + _vq_h - 1.55, 'per step, per rule', fs=4.35, w='bold', c='#9DB8E8')
ax.plot([92.7, 104.1], [_vq_y + _vq_h - 2.85, _vq_y + _vq_h - 2.85], color='#7A8A9A', lw=LW_HAIR, zorder=5)
_KW, _TX, _PI = '#61AFEF', '#E8EBF0', '#E5C07B'
_sql_y = _vq_y + _vq_h - 4.7
txt(92.7, _sql_y, 'WHERE', fs=3.65, ha='left', family='monospace', c=_KW, w='bold')
txt(96.55, _sql_y, '$\\pi_{topo}\\wedge\\pi_{precond}$', fs=3.5, ha='left', c=_PI, w='bold')
txt(92.7, _sql_y - 2.5, 'GROUP BY', fs=3.65, ha='left', family='monospace', c=_KW, w='bold')
txt(93.3, _sql_y - 4.65, 'param, step', fs=3.65, ha='left', family='monospace', c=_TX, w='bold')
txt(92.7, _sql_y - 7.0, 'HAVING', fs=3.65, ha='left', family='monospace', c=_KW, w='bold')
txt(96.7, _sql_y - 7.0, '$\\pi_{schema}$', fs=3.5, ha='left', c=_PI, w='bold')
txt(93.3, _sql_y - 9.05, 'violated', fs=3.6, ha='left', family='monospace', c=_TX, w='bold')
txt(91.0, _oy + 8.6, 'empty result = holds · non-empty IS the violation set',
    fs=3.6, style='italic')
txt(91.0, _oy + 5.2, 'coarse$\\rightarrow$fine: checksum$\\rightarrow$module$\\rightarrow$param$\\rightarrow$bitwise',
    fs=3.4, c=GRAY)
arrow((106.5, _bridge_y), (108.4, _bridge_y), c=RED_DEEP, lw=LW_THICK, ms=11, halo=True)

# ================= Violation panel (= ON block y/h, mirrors left online output) =================
shadow(RX, ON_Y, RW, ON_H)
box(RX, ON_Y, RW, ON_H, fc=VIOL_BG, ec=RED, lw=LW_MID, r=1.1, ls=(0, (3.0, 2.0)))
txt(_rc, ON_Y + ON_H - 1.8, 'violation report', fs=5.25, w='bold', c=RED)
ax.plot([_rc - 8.5, _rc + 8.5], [ON_Y + ON_H - 3.1, ON_Y + ON_H - 3.1],
        color=RED, lw=LW_HAIR, alpha=0.7, zorder=5)
_tx, _cols = RX + 2.7, [('param', 8.6), ('rank', 3.4), ('cksum', 7.2)]
_rows = [('router.weight', '0', 'cksum$_A$', False), ('router.weight', '1', 'cksum$_B$', True)]
_ty, _rh = ON_Y + ON_H - 4.0, 2.35
_cx = _tx
for _h, _wd in _cols:
    ax.add_patch(Rectangle((_cx, _ty - _rh), _wd, _rh, fc='#F5F5F5', ec='#B3B3B3', lw=LW_HAIR, zorder=4))
    txt(_cx + _wd / 2, _ty - _rh / 2, _h, fs=3.45, c=GRAY, z=5)
    _cx += _wd
for _r, (_p, _rk, _ck, _bad) in enumerate(_rows):
    ...
    for _v, (_h, _wd) in zip((_p, _rk, _ck), _cols):
        ...
        txt(_cx + _wd / 2, _yy + _rh / 2, _v, fs=3.45, family='monospace',
            c=RED if _bad else DARK, z=5)
        _cx += _wd
txt(_rc, ON_Y + 14.2, '$\\mathbf{\\times}$ divergent checksums', fs=3.8, c=RED)
for _k, _it in enumerate(['violating constraint', 'affected parameter',
                          'divergent ranks', 'first violating step']):
    _yy = ON_Y + 11.6 - _k * 2.7
    checkbox(RX + 3.0, _yy - 0.55, 1.2, RED)
    txt(RX + 5.1, _yy, _it, fs=3.85, ha='left')
# viol → online output: leave panel left-bottom, under-arc in pad, tip into report body
arrow((RX, ON_Y + 1.1), (LX + LW, ON_Y + ON_H * 0.38), c=DARK, lw=LW_THICK, ms=12,
      cs='arc3,rad=0.20', halo=True)

# ================= left bay: online output (= ON block y/h, mirrors offline inputs) =================
dashed_group(LX, ON_Y, LW, ON_H, '#FBF0EC', label=None, lc=ORANGE_SOFT)
txt(LX + 1.2, ON_Y + ON_H - 1.7, 'online output', fs=5.25, w='bold', c=ORANGE, ha='left')
folded_doc(LX + 1.1, ON_Y + 5.6, _doc_w, ON_H - 10.0,
           ...
           hl={0: RED, 2: RED})
txt(CX, ON_Y + 3.6, 'empty result = holds', fs=3.85, style='italic', c=GRAY)
txt(CX, ON_Y + 1.55, 'non-empty = violation set', fs=3.85, style='italic', c=RED)

# ================= legend (tight under outer) =================
_leg_y = _outer_y - 4.55   # legend glued under outer (no floating gap)
ax.add_patch(FancyBboxPatch((25, _leg_y), 117.5, 3.7,
             boxstyle='round,pad=0,rounding_size=0.55',
             fc='#F4F5F7', ec='#B8B8B8', lw=LW_HAIR, zorder=2))
_leg_mid = _leg_y + 1.85
...
fig.savefig('fig_overview_v24.pdf')
fig.savefig('/tmp/fig_overview_v24.png', dpi=240)
print('done v24')
