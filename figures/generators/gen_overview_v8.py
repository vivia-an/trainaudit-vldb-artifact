#!/usr/bin/env python3
"""QUITE-genome layout (v8): outer container + left Input/Output bays (user-approved raster,
...
# ================= INPUT bay (left, outside) =================
shadow(1, 56, 22, 36, r=1.4)
box(1, 56, 22, 36, fc='white', ec='#141414', lw=1.1, r=1.4)
txt(2.5, 89.6, 'Input', fs=5.6, w='bold', ha='left')
chip(4.4, 85.9, 1.5, BLUE)
icon_book(4.4, 85.9, 0.85, 'white')
txt(6.8, 85.9, 'Pattern Catalog', fs=4.6, w='bold', c=BLUE, ha='left')
txt(2.5, 82.4, '16 templates · from 392 real\nsilent errors · 13 fault classes', fs=3.7, ha='left', ls=1.3)
txt(2.5, 77.3, 'dtype preservation · cross-rank\nreplication · scaling consistency ·\ninvocation frequency · state\nrestoration · counter consistency',
    fs=3.2, c=GRAY, ha='left', ls=1.3)
chip(4.4, 71.0, 1.5, BLUE)
icon_doc(4.4, 71.0, 0.8, 'white')
txt(6.8, 71.0, 'Framework source & docs', fs=4.0, w='bold', c=BLUE, ha='left')
txt(2.5, 67.6, 'Megatron-LM · DeepSpeed ·\nOLMo · OLMo-core', fs=3.8, ha='left', ls=1.3)
txt(2.5, 61.7, 'silent-vs-loud filter:\nno explicit raise or assert', fs=3.9, w='bold', c=RED,
    ha='left', ls=1.3, style='italic')
arrow((23.4, 74), (28.6, 79.5), c=BLUE, lw=1.3)

# ================= Invariant Miner (offline row) =================
shadow(27, 53.5, 79, 33)
box(27, 53.5, 79, 33, fc='#f9fbfe', ec=DARK, lw=1.0)
badge(30.6, 83.9, '1', c=BLUE)
...
txt(76.5, 43.5, 'deterministic SQL checking — no LLM', fs=3.8,
    style='italic', c=GRAY, ha='left')
box(77, 27, 13, 14, fc='#fbfbfb', ec=DARK, lw=0.6)
...
# ================= OUTPUT bay (left, outside) =================
shadow(1, 8, 22, 34, r=1.4)
box(1, 8, 22, 34, fc='white', ec='#141414', lw=1.1, r=1.4)
txt(2.5, 39.4, 'System Output', fs=5.4, w='bold', ha='left')
txt(2.5, 36.6, 'the final report names:', fs=3.7, style='italic', c=GRAY, ha='left')
for _k, _it in enumerate(['violating constraint', 'affected parameter',
                          'divergent ranks', 'first violating step']):
    _yy = 33.4 - _k * 2.7
    checkbox(2.7, _yy - 0.6, 1.2, '#39404d')
    txt(4.7, _yy, _it, fs=3.9, ha='left')
ax.plot([2.5, 21.5], [21.6, 21.6], color='#cccccc', lw=0.5, zorder=4)
txt(12, 18.9, 'router.weight divergence\nintercepted in production', fs=3.8, ls=1.3)
txt(12, 14.4, '$\\approx$43,000 GPU-hours saved', fs=4.5, w='bold', c=RED)
txt(12, 11.6, '(256$\\times$H200 production run)', fs=3.5, c=GRAY)

# ================= LLM agents row (QUITE genome, offline only) =================
txt(25.5, 4.0, 'LLM roles (offline only):', fs=4.3, w='bold', ha='left')
roles = [(49, 'gap analysis'), (63, 'evidence retrieval'), (80, 'synthesis'),
         (91.5, 'persistence'), (103.5, 'reporting')]
for rx, rl in roles:
    icon_person(rx, 4.0, 1.05, '#39404d')
    txt(rx + 1.8, 4.0, rl, fs=3.7, ha='left')
txt(142.5, 4.0, 'the online path contains no LLM call', fs=3.9, style='italic', c=GRAY, ha='right')

fig.savefig('fig_overview_v8.pdf')
fig.savefig('/tmp/fig_overview_v8.png', dpi=220)
print('done')
