#!/usr/bin/env python3
"""v13 = v12 bloodline-safe + layout de-overlap + texture.
Same label set as v11/v12; reflow Miner/Verifier/violation/arrows only.
"""
import matplotlib
matplotlib.use('Agg')
...
box(25, 8.0, 118, 89, fc='#FFFEFB', ec='#1A1A1A', lw=1.85, r=1.6, z=1.8)
txt(84, 94.8, 'TrainAudit', fs=8.0, w='bold', c=BLUE)
ax.plot([38, 74.5], [94.8, 94.8], color=BLUE, lw=0.55, zorder=4)
ax.plot([93.5, 130], [94.8, 94.8], color=BLUE, lw=0.55, zorder=4)

# lane header bars (clear of crossing arrows)
ax.add_patch(Rectangle((27, 90.6), 79, 2.1, fc=BLUE_BG, ec='none', zorder=2))
txt(27.5, 91.65, 'OFFLINE — mine once per framework', fs=5.7, w='bold', c=BLUE, ha='left')
ax.add_patch(Rectangle((27, 49.6), 79, 2.1, fc=ORANGE_BG, ec='none', zorder=2))
...
arrow((23.0, 81), (28.6, 81), c=BLUE, lw=1.6, ms=9, halo=True)

# ================= Invariant Miner (offline row) =================
# Bands (top→bottom): title | plus-compose | label-strip+steps | reject | FSM | creds
shadow(27, 54.5, 79, 35.5)
box(27, 54.5, 79, 35.5, fc='#f9fbfe', ec=DARK, lw=1.0)
badge(30.6, 87.2, '1')
txt(33.4, 87.2, 'Invariant Miner', fs=6.2, w='bold', ha='left')
...
for px, ic, lb in plus_cards:
    box(px, 80.6, 15.5, 4.6, fc='white', ec=BLUE_SOFT, lw=0.55, r=0.7)
    ic(px + 2.6, 82.9, 0.8, BLUE)
    txt(px + 9.4, 82.9, lb, fs=3.2, ls=1.12)
for gx in (46.2, 65.2, 84.2):
    txt(gx, 82.9, '+', fs=6.2, w='bold', c=GRAY)
arrow((65.5, 80.4), (65.5, 79.5), c=BLUE, lw=0.85)

dashed_group(28.2, 66.0, 38.2, 13.2, LAVENDER, label=None, lc='#7B6B95')
dashed_group(66.8, 66.0, 37.8, 13.2, MINT, label=None, lc='#5B8A5B')
box(28.8, 76.6, 12.5, 2.0, fc=LAVENDER, ec='#7B6B95', lw=0.45, r=0.4, ls=(0, (2, 1.5)))
txt(35.05, 77.6, 'instantiate', fs=3.5, w='bold', c='#7B6B95')
box(67.4, 76.6, 16.5, 2.0, fc=MINT, ec='#5B8A5B', lw=0.45, r=0.4, ls=(0, (2, 1.5)))
txt(75.65, 77.6, 'verify (decisive)', fs=3.5, w='bold', c='#5B8A5B')
icon_brain(42.2, 77.6, 0.7, '#7B6B95')
...
for x, name, sub, ec, lw, fc in steps:
    box(x, 66.6, 16.5, 9.5, fc=fc, ec=ec, lw=lw)
    txt(x + (9.3 if name == 'Construct' else 8.9), 73.9, name, fs=5.1, w='bold', c=ec)
    ax.plot([x + 2.2, x + 14.3], [72.8, 72.8], color=ec, lw=0.45, alpha=0.65, zorder=5)
...
for x0 in (45.8, 65.05, 84.3):
    arrow((x0, 71.35), (x0 + 2.2, 71.35), c=BLUE, lw=0.9)

# reject loop tight under step cards; FSM band fully below
arrow((95.0, 66.45), (95.0, 65.15), c=RED, lw=0.75, ls=(0, (3, 2)), style='-')
arrow((95.0, 65.15), (37.3, 65.15), c=RED, lw=0.75, ls=(0, (3, 2)), style='-')
arrow((37.3, 65.15), (37.3, 66.35), c=RED, lw=0.75, ls=(0, (3, 2)))
...
for k, (sn, lb, cc) in enumerate(fsm):
    ...
    if k < 4:
        arrow((cx + 1.65, 59.7), (cx + 6.55, 59.7), c='#9aa5b5', lw=0.55, ms=4)
...
arrow((103.3, 71.35), (110.5, 78.0), c=BLUE, lw=1.6, ms=9, cs='arc3,rad=-0.18', halo=True)
...
box(108.5, 46.5, 32.5, 43.5, fc=HUB_BG, ec=DARK, lw=1.0)
cyl(120.5, 83.5, 8.5, 5.0, fc='#e9e2d2', ec=DARK)
txt(124.7, 80.6, 'Verified Constraint Library $\\mathcal{P}$', fs=4.8, w='bold')
txt(124.7, 78.4, 'guarded relational constraints', fs=3.8, style='italic', c=GRAY)
shadow(110.5, 55.5, 28.5, 21.0, r=1.0)
box(110.5, 55.5, 28.5, 21.0, fc='white', ec=DARK, lw=0.7, r=1.0)
...
arrow((108.5, 56.5), (63.5, 48.5), c=ORANGE, lw=0.95, cs='arc3,rad=0.05')
# compile arrow into Verifier right edge (not over title bar)
arrow((124.7, 46.3), (106.4, 34.5), c=ORANGE, lw=1.55, ms=9, cs='arc3,rad=0.18', halo=True)

# ================= Data Collector (online row) =================
shadow(27, 16.0, 36.5, 33.5)
box(27, 16.0, 36.5, 33.5, fc='#fffdf9', ec=DARK, lw=1.0)
badge(30.6, 46.8, '2')
txt(33.4, 46.8, 'Data Collector', fs=6.0, w='bold', ha='left')
...
box(28.8, 18.6, 33, 6.2, fc='#fdf8f0', ec=ORANGE, lw=0.7)
...
for _yy in (31.2, 34.2):
    ax.plot([65.9, 73.0], [_yy, _yy], color=ORANGE, lw=0.4, alpha=0.5, zorder=4)
txt(69.4, 25.0, 'DuckDB\ntrace DB', fs=4.0, w='bold', c=ORANGE, ls=1.15)
txt(69.4, 20.8, 'two-step\nsliding window', fs=3.3, c=GRAY, ls=1.15)
arrow((63.7, 33.0), (64.8, 33.0), c=ORANGE, lw=1.05)
arrow((74.0, 33.0), (75.2, 33.0), c=ORANGE, lw=1.05)

# ================= Verifier =================
shadow(75.5, 16.0, 31.0, 33.5)
box(75.5, 16.0, 31.0, 33.5, fc='#fffdf9', ec=DARK, lw=1.0)
badge(79.1, 46.8, '3')
txt(81.9, 46.8, 'Verifier', fs=6.0, w='bold', ha='left')
chip(92.0, 46.8, 1.45, ORANGE)
icon_search(92.0, 46.8, 0.85, 'white')
txt(76.5, 44.2, 'deterministic SQL checking — no LLM', fs=3.6,
    style='italic', c=GRAY, ha='left')
# equal-height subcards with padding
box(76.8, 27.2, 13.6, 14.6, fc=MINT, ec='#5B8A5B', lw=0.65, ls=(0, (2.5, 1.8)))
txt(83.6, 40.2, 'at job start', fs=4.1, w='bold')
ax.plot([78.0, 89.2], [38.9, 38.9], color='#555b63', lw=0.5, zorder=5)
txt(83.6, 33.0, 'extract topology $\\tau$\ntopology-aware\npruning · lineage:\nreplicated vs.\nsharded', fs=3.45, ls=1.35)
box(91.4, 27.2, 14.0, 14.6, fc='#2b313d', ec='#1f242e', lw=0.7)
txt(98.4, 40.2, 'per step, per rule', fs=4.0, w='bold', c='#9db8e8')
ax.plot([92.6, 104.2], [38.9, 38.9], color='#4a5568', lw=0.5, zorder=5)
...
txt(91.0, 20.6, 'coarse$\\rightarrow$fine: checksum$\\rightarrow$module$\\rightarrow$param$\\rightarrow$bitwise',
    fs=3.25, c=GRAY)
arrow((106.6, 34.5), (108.2, 34.5), c=RED, lw=1.5, ms=9, halo=True)

# ================= Violation panel =================
shadow(108.5, 11.0, 32.5, 33.0)
box(108.5, 11.0, 32.5, 33.0, fc='#FDF0EF', ec=RED, lw=1.0, ls=(0, (3.5, 2)))
txt(124.7, 41.2, 'violation report', fs=5.0, w='bold', c=RED)
ax.plot([116.5, 132.8], [39.8, 39.8], color=RED, lw=0.5, zorder=5)
...
arrow((108.5, 13.2), (23.5, 18.5), c=DARK, lw=1.5, ms=9, cs='arc3,rad=-0.05', halo=True)
...
# ================= legend =================
ax.add_patch(Rectangle((25, 2.4), 117.5, 3.8, fc='#FAFAFA', ec='#D0D0D0', lw=0.4, zorder=2))
txt(26.5, 4.3, 'Legend', fs=4.2, w='bold', ha='left')
ax.plot([36, 43], [4.3, 4.3], color=BLUE, lw=1.55)
txt(44.5, 4.3, 'offline mining', fs=3.9, ha='left')
ax.plot([62, 69], [4.3, 4.3], color=ORANGE, lw=1.55)
txt(70.5, 4.3, 'online checking', fs=3.9, ha='left')
ax.plot([92, 99], [4.3, 4.3], color=RED, lw=1.25, ls=(0, (3, 2)))
...
fig.savefig('fig_overview_v13.pdf')
fig.savefig('/tmp/fig_overview_v13.png', dpi=220)
print('done')
