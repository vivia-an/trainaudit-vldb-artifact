...
| R3 | Single-blind, names on page 1 | "VLDB is a single-blind conference. Therefore, authors MUST include their names and affiliations on the first page of the manuscript." | **OK for `main.tex`** — 7 authors + affiliations at `main.tex:126–183`. **Not for the supplement**: `appendix.tex` is still `[sigconf,anonymous]` and renders "Anonymous Author(s)" (O29) | de-anonymise `appendix.tex` |
