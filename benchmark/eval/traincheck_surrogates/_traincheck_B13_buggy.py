...
import os
os.environ['ML_DAIKON_OUTPUT_DIR'] = "/tmp/tc_B13/trace_buggy"
...
proxy_config.__dict__.update({'proxy_log_dir': '/tmp/tc_B13/trace_buggy/proxy_log.json'})
...
def main():
    annotate_stage('init')
    torch.manual_seed(0)
    model = nn.Sequential(nn.Linear(8, 8), BrokenResidualBlock(8), BrokenResidualBlock(8), nn.Linear(8, 4))
