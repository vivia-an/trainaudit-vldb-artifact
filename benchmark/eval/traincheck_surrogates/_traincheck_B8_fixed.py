...
import os
os.environ['ML_DAIKON_OUTPUT_DIR'] = "/tmp/tc_B8/trace_fixed"
...
proxy_config.__dict__.update({'proxy_log_dir': '/tmp/tc_B8/trace_fixed/proxy_log.json'})
...
class MoEBlock(nn.Module):
    ...
    def __init__(self, hidden=8, n_experts=2, declared_ep_size=2):
        super().__init__()
        assert n_experts == declared_ep_size, f'EP size mismatch: {n_experts} != {declared_ep_size}'
        self.experts = nn.ModuleList([nn.Linear(hidden, hidden) for _ in range(n_experts)])
...
def main():
    annotate_stage('init')
    torch.manual_seed(0)
    model = nn.Sequential(nn.Linear(8, 8), MoEBlock(8, n_experts=2, declared_ep_size=2), nn.Linear(8, 4))
