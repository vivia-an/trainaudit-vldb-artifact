...
import os
os.environ['ML_DAIKON_OUTPUT_DIR'] = "/tmp/tc_O-NEW-1/trace_buggy"
...
proxy_config.__dict__.update({'proxy_log_dir': '/tmp/tc_O-NEW-1/trace_buggy/proxy_log.json'})
...
def main():
    annotate_stage('init')
    torch.manual_seed(0)
    model = nn.Sequential(nn.Linear(8, 16), BrokenRMSNorm(16), nn.GELU(), nn.Linear(16, 4))
