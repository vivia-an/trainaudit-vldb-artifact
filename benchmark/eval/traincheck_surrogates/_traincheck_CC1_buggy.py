...
import os
os.environ['ML_DAIKON_OUTPUT_DIR'] = "/tmp/tc_CC1/trace_buggy"
...
proxy_config.__dict__.update({'proxy_log_dir': '/tmp/tc_CC1/trace_buggy/proxy_log.json'})
...
class FakeOptimWrapper:
    ...
    def __init__(self, optimizer, zero_stage):
        ...
        self.has_partitioned_state = False
...
def main():
    ...
    model = Proxy(model, recurse=True, logdir=proxy_config.proxy_log_dir, var_name='model')
    inner_opt = torch.optim.AdamW(model.parameters(), lr=0.001)
    opt = FakeOptimWrapper(inner_opt, zero_stage=config['zero_stage'])
    print(f'[CC1_buggy] config.zero_stage={config['zero_stage']}  optimizer.has_partitioned_state={opt.has_partitioned_state}  ⟹ P10 violation: zero>{0} but state not partitioned')
