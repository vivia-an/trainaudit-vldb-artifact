...
_DEFAULT_SHARED_TMP = "/volume/qscai/cqs/temp/trainaudit_regr"
...
def run_driver_on_commit(framework_dir: Path, commit: str,
    ...
    Stages a self-contained bash script to a shared-filesystem path
    (default /volume/qscai/cqs/temp/trainaudit_regr/) and runs it via
    `ssh <host> 'bash -l <script>'`. This avoids local shell quoting of
    `$RANDOM`, `${PYTHONPATH:-}`, etc."""
    ...
    repo_root_str = str(repo_root) if repo_root else \
        os.environ.get("TRAINAUDIT_REPO",
                       "/volume/qscai/cqs/workspace/paper/sdc_llm_icml_2025")
