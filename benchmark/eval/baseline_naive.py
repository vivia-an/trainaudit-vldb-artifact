"""E2: Naïve metric-monitoring baseline.

Tracks four scalar signals during D1 surrogate training and flags any anomaly
as a detection. This is the strawman baseline that paper §4.1 contrasts with
TrainAudit's semantic invariants.
...
Modes:
  --mode synthetic  : run in-process Naïve runners on D1 (28 instances)
  --mode subprocess : run benchmark/bugs/<id>/naive_run.sh (23 real bugs, GPU)

Output schema (per doc 26 §4.4):
...
def run_subset(subset_path: Path, manifest_path: Path,
    ...
    # Drop fixed-phase rows for bugs that never had a fixed pair (per doc 26
    # §3.1): synthetic_14.json has all 14 bugs with both phases, so we keep
    # all 28 rows. If subset signals 'no fixed', the runner returns CLEAN
...
def main():
    ...
    ap.add_argument("--mode", choices=["synthetic", "subprocess"],
                    default="synthetic")
    ...
    if args.mode == "synthetic":
        rows = run_subset(Path(args.subset), Path(args.manifest), set_label="D1")
    ...
    if args.mode == "subprocess":
        print("subprocess mode not yet implemented (D7: 23 real bugs)")
        sys.exit(2)
