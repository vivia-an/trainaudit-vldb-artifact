#!/bin/bash
#
# 测试 backward前 DP 主权重 bitwise-level 一致性约束的错误注入
#
# 约束名称：backward前DP主权重bitwise-level一致性检查
# 用途：通过在 backward 前注入极小参数修改，验证 SDCCheck 约束检测系统能否检测 bitwise 不一致
# 预期：训练正常运行，但 cksum 校验会发现 bitwise-level 不一致
#
# 注意：
# 1. 注入发生在 schedules.py 的 backward_step() 开始时
# 2. 使用 [corrupt-cksum-before-bwd] 日志标识
...
echo "=========================================="
echo "测试 backward前 DP 参数 cksum 一致性错误注入"
echo "=========================================="

# 清理之前的数据
export VTIMELINE_LOGGER_DIR=/volume/qscai/lsk/Megatron-LM/cksum_before_backward_bitwise_level_test_db
rm -rf $VTIMELINE_LOGGER_DIR
mkdir -p $VTIMELINE_LOGGER_DIR

# ========================================
# 错误注入配置（cksum_before_backward 注入）
# ========================================
export MEGATRON_INJECT_PARAM_CORRUPTION=1
export MEGATRON_CORRUPT_OP=cksum_before_backward
...
echo "开始训练..."
echo "预期行为："
echo "  1. 训练可能崩溃（all-reduce 可能检测到参数不一致）或正常运行"
echo "  2. 在 step 1，DP rank 0 的某参数 cksum 被修改"
echo "  3. model-before-backward 阶段，DP rank 0 和 1 的 cksum 不一致"
echo ""
echo "日志标识："
echo "  [corrupt-cksum-before-bwd] - schedules.py backward 前的注入日志"
echo ""
...
python -m torch.distributed.run \
    ...
    2>&1 | tee cksum_before_backward_injection_test.log || true
...
if [ $EXIT_CODE -eq 0 ]; then
    echo "✓ 训练正常完成"
    echo "  cksum 注入可能未导致崩溃，但 cksum 不一致已被记录"
else
    echo "⚠ 训练失败（退出码: ${EXIT_CODE}）"
    echo "  这可能是因为 cksum 不一致导致的 all-reduce 错误"
fi
...
echo ""
echo "检查日志中的错误注入信息..."
if [ -f "cksum_before_backward_injection_test.log" ]; then
    echo ""
    echo "--- cksum before backward 注入日志（[corrupt-cksum-before-bwd]）---"
    grep "\[corrupt-cksum-before-bwd\]" cksum_before_backward_injection_test.log | head -30
    
    if [ $? -ne 0 ]; then
        echo "⚠️  未找到 [corrupt-cksum-before-bwd] 日志"
    fi
    echo ""
    
    # 统计注入次数
    INJECT_COUNT=$(grep -c "Injection completed" cksum_before_backward_injection_test.log || echo "0")
    echo "✓ 检测到 $INJECT_COUNT 次成功注入"
    
    # 检查修改的参数数量
    MODIFIED_COUNT=$(grep -c "Modified param" cksum_before_backward_injection_test.log || echo "0")
    echo "✓ 修改了 $MODIFIED_COUNT 个参数"
fi
...
# 创建验证脚本
cat > check_cksum_before_backward_injection.py << 'PYTHON_EOF'
import duckdb
import os
import sys

db_dir = os.environ.get("VTIMELINE_LOGGER_DIR", "/volume/qscai/lsk/Megatron-LM/cksum_before_backward_test_db") + "/Collector"
...
for dp in [0, 1]:
    ...
    if result:
        ...
        for row in result:
            ...
            print(f"    name={row[2]}")
            print(f"    cksum={row[3][:20] if row[3] else 'N/A'}...")
...
if 0 in results and 1 in results:
    ...
    if mismatch_count > 0:
        print("")
        print("🎉 验证成功：检测到注入导致的 cksum 不一致！")
        sys.exit(0)
    else:
        print("")
        print("⚠️  验证失败：所有 cksum 都一致，注入可能未生效")
        sys.exit(1)
else:
...
echo "运行验证脚本..."
python check_cksum_before_backward_injection.py
VERIFY_EXIT_CODE=$?
...
if [ $VERIFY_EXIT_CODE -eq 0 ]; then
    echo "✅ 注入实验成功！"
    echo "   成功检测到 DP rank 间的 cksum 不一致"
else
    echo "⚠️  注入实验可能未成功"
    echo "   请检查日志和数据库内容"
fi
...
# 清理临时文件
rm -f check_cksum_before_backward_injection.py

echo ""
echo "=========================================="
echo "血缘细节总结"
echo "=========================================="
echo "  约束名称: backward前DP主权重bitwise-level一致性检查"
echo "  检测阶段: model-before-backward"
echo "  检测逻辑: 主权重参数在不同 DP rank 间应 bitwise 完全一致"
echo ""
echo "  注入位置: schedules.py::backward_step() → dump_model() 之前"
echo "  日志标识: [corrupt-cksum-before-bwd]"
echo "  数据收集: MegatronCollector.dump_model(\"model-before-backward\")"
echo "  验证字段: cksum (SHA256 校验和)"
echo ""
echo "  bitwise-level 检测说明:"
echo "    - 使用极小的 delta (1e-7) 修改参数"
echo "    - 参数值改变微乎其微，不影响训练收敛"
echo "    - 但 SHA256 校验和会完全不同"
echo "    - 这模拟了 silent data corruption 场景"
echo ""
