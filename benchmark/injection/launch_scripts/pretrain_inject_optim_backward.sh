#!/bin/bash
#
# 测试 backward后 DP optimizer_state 一致性约束的错误注入
#
# 用途：通过在 backward 后注入 optimizer_state 不一致错误，验证 SDCCheck 约束检测系统能否正确识别
# 预期：训练正常运行，但在 optimizer-state-after-backward 阶段，不同DP rank的optimizer state不一致
#
# 注意：
# 1. 注入发生在 schedules.py 的 backward_step() 之后
# 2. 使用 [corrupt-optim-backward] 日志标识
# 3. 数据收集使用 dump_optimizer_state("optimizer-state-after-backward") 方法
#
...
echo "=========================================="
echo "测试 backward后 DP optimizer_state 一致性错误注入"
echo "=========================================="

# 清理之前的数据
export VTIMELINE_LOGGER_DIR=/volume/qscai/lsk/Megatron-LM/optim_backward_test_db
rm -rf $VTIMELINE_LOGGER_DIR
mkdir -p $VTIMELINE_LOGGER_DIR

# ========================================
# 错误注入配置（optimizer_state_backward 注入）
# ========================================
export MEGATRON_INJECT_PARAM_CORRUPTION=1
export MEGATRON_CORRUPT_OP=optimizer_state_backward
export MEGATRON_CORRUPT_DP_RANK=0
export MEGATRON_CORRUPT_STEP=2  # 在第2步注入（第1步optimizer state已创建）
...
echo "  3. optimizer-state-after-backward 阶段，DP rank 0 和 1 的 state 不一致"
echo ""
echo "日志标识："
echo "  [corrupt-optim-backward] - schedules.py backward 后的注入日志"
echo ""
...
python -m torch.distributed.run \
    ...
    2>&1 | tee optim_backward_injection_test.log
...
echo ""
echo "检查日志中的错误注入信息..."
if [ -f "optim_backward_injection_test.log" ]; then
    echo ""
    echo "--- optimizer_state backward 注入日志（[corrupt-optim-backward]）---"
    grep "\[corrupt-optim-backward\]" optim_backward_injection_test.log | head -30
    
    if [ $? -ne 0 ]; then
        echo "⚠️  未找到 [corrupt-optim-backward] 日志"
    fi
    echo ""
    
    # 统计注入次数
    INJECT_COUNT=$(grep -c "Injection completed" optim_backward_injection_test.log || echo "0")
    echo "✓ 检测到 $INJECT_COUNT 次成功注入"
    
    # 检查修改的 state 数量
    MODIFIED_COUNT=$(grep -c "Modified exp_avg" optim_backward_injection_test.log || echo "0")
...
# 创建验证脚本
cat > check_optim_backward_injection.py << 'PYTHON_EOF'
import duckdb
import os
import sys

db_dir = os.environ.get("VTIMELINE_LOGGER_DIR", "/volume/qscai/lsk/Megatron-LM/optim_backward_test_db") + "/Collector"
...
for dp in [0, 1]:
    ...
    # 查询 optimizer-state-after-backward 阶段的数据
    target_stage = 'optimizer-state-after-backward'
    ...
    if result:
        ...
        print(f"  ❌ 所有 optimizer-state 阶段都没有找到数据")
        results[dp] = {}
...
if 0 in results and 1 in results:
    ...
    for key in sorted(all_keys):
        ...
        if cksum0 != cksum1:
            mismatch_count += 1
            print(f"❌ 不一致: {key}")
            print(f"   DP0 cksum: {cksum0[:20]}..." if cksum0 != "N/A" else f"   DP0 cksum: {cksum0}")
            print(f"   DP1 cksum: {cksum1[:20]}..." if cksum1 != "N/A" else f"   DP1 cksum: {cksum1}")
        else:
            match_count += 1
...
echo "运行验证脚本..."
python check_optim_backward_injection.py
VERIFY_EXIT_CODE=$?
...
# 清理临时文件
rm -f check_optim_backward_injection.py

echo ""
echo "关键信息："
echo "  - 约束名称: backward后DP optimizer_state一致性检查"
echo "  - 注入位置: schedules.py::backward_step() 之后"
echo "  - 日志标识: [corrupt-optim-backward]"
echo "  - 数据收集: dump_optimizer_state(\"optimizer-state-after-backward\")"
echo "  - 验证 stage: optimizer-state-after-backward"
